...
OUTCOMES = frozenset({"PASS", "FAIL", "UNKNOWN", "DRIFT", "BLOCKED", "COMPLETE"})
...
class ProcessTransitionOutcome:
    """Evidence-bound feedback from downstream processing into one Process transition."""
    process_id: str
    process_ir_digest: str
    transition_id: str
    outcome: str
    ...
    def validate(self) -> "ProcessTransitionOutcome":
        ...
        if type(self.consequential) is not bool:
            raise ProcessActionContractError("consequential must be boolean")
        ...
        if self.consequential and self.outcome == "PASS":
            required = (self.action_ref, self.admission_ref, self.effect_ref, self.observation_ref, self.reconciliation_ref, self.currentness_basis_ref)
            if any(not item for item in required):
                raise ProcessActionContractError("consequential PASS requires action, admission, effect, observation, reconciliation and currentness refs")

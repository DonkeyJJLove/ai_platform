"""Immutable contracts for the LION Local Swarm P0 Docker polygon.

The module describes TEST_ONLY materialization intent and evidence.  It does not
mint authority and deliberately contains no Docker client or subprocess calls.
"""
...
class MissionCapsule:
    ...
    def validate(self, *, now: datetime | None = None) -> "MissionCapsule":
        ...
        _sha256(self.capsule_digest, "capsule_digest")
        issued = _iso8601(self.issued_at, "issued_at")
        expires = _iso8601(self.expires_at, "expires_at")
        if expires <= issued:
    ...
    def canonical_dict(self) -> dict[str, Any]:
        ...
        value["capsule_digest"] = self.capsule_digest
        return value
...
class DockerDroneBinding:
    ...
    def validate(self) -> "DockerDroneBinding":
        ...
        _sha256(self.lease_id, "lease_id")
        _container_name(self.container_name, "container_name")
        _sha256(self.image_digest, "image_digest")
        _sha256(self.capsule_digest, "capsule_digest")
        return self
...
class DockerFleetPlan:
    ...
    def validate(self) -> "DockerFleetPlan":
        if self.schema_version != SCHEMA_VERSION or self.trust_class != "TEST_ONLY":
            raise DockerFleetPolygonContractError("fleet schema/trust invalid")
        for name in ("mission_id", "fleet_id", "run_id"):
            _identifier(getattr(self, name), name)
        if self.repository != "DonkeyJJLove/ai_platform":
            raise DockerFleetPolygonContractError("P0 repository mismatch")
        _sha40(self.source_head, "source_head")
        _sha40(self.source_tree, "source_tree")
        _container_name(self.network_name, "network_name")
        _text(self.image_reference, "image_reference", limit=512)
        _sha256(self.image_digest, "image_digest")
        _sha256(self.runtime_profile_digest, "runtime_profile_digest")
        if type(self.drones) is not tuple or len(self.drones) != 5:
            raise DockerFleetPolygonContractError("P0 requires exactly five drones")
        for drone in self.drones:
            drone.validate()
            if drone.image_digest != self.image_digest:
                raise DockerFleetPolygonContractError("drone image binding mismatch")
        if tuple(sorted(d.role for d in self.drones)) != tuple(sorted(P0_ROLES)):
            raise DockerFleetPolygonContractError("P0 roles incomplete")
        for attr in ("drone_id", "container_name", "lease_id"):
            values = [getattr(d, attr) for d in self.drones]
            if len(values) != len(set(values)):
                raise DockerFleetPolygonContractError(f"duplicate {attr}")
        return self

    def canonical_dict(self) -> dict[str, Any]:
        self.validate()
        value = asdict(self)
        value["drones"] = [d.canonical_dict() for d in self.drones]
        return value

    def digest(self) -> str:
        return digest_domain(b"LION/DOCKER-FLEET-PLAN/P0", self.canonical_dict())
...
class DroneResult:
    ...
    def validate(self) -> "DroneResult":
        if self.schema_version != SCHEMA_VERSION:
            raise DockerFleetPolygonContractError("result schema mismatch")
        _sha256(self.result_id, "result_id")
        for name in ("mission_id", "fleet_id", "drone_id", "work_unit_id"):
            _identifier(getattr(self, name), name)
        if self.role not in P0_ROLES or self.status not in {"SUCCEEDED", "FAILED", "DENIED"}:
            raise DockerFleetPolygonContractError("result role/status invalid")
        _positive_int(self.generation, "generation")
        _sha256(self.input_digest, "input_digest")
        _sha256(self.result_digest, "result_digest")
        _iso8601(self.started_at, "started_at")
        _iso8601(self.completed_at, "completed_at")
        if type(self.evidence) is not tuple or len(set(self.evidence)) != len(self.evidence):
            raise DockerFleetPolygonContractError("result evidence invalid")
        expected = digest_domain(b"LION/DRONE-RESULT-BODY/P0", dict(self.result))
        if expected != self.result_digest:
            raise DockerFleetPolygonContractError("result digest mismatch")
        return self
...
class ObservedContainer:
    ...
    def validate(self) -> "ObservedContainer":
        if not isinstance(self.container_id, str) or len(self.container_id) < 12 or not re.fullmatch(r"[0-9a-f]+", self.container_id):
            raise DockerFleetPolygonContractError("container_id invalid")
        _container_name(self.container_name, "container_name")
        _sha256(self.image_digest, "image_digest")
        if not isinstance(self.labels, Mapping):
            raise DockerFleetPolygonContractError("labels invalid")
        if self.user != "65532:65532" or not self.read_only_rootfs:
            raise DockerFleetPolygonContractError("container hardening mismatch")
        if self.cap_drop != ("ALL",) or "no-new-privileges" not in self.security_opt:
            raise DockerFleetPolygonContractError("container privilege boundary mismatch")
        for name in ("pids_limit", "memory_limit_bytes", "nano_cpus"):
            _positive_int(getattr(self, name), name)
        _container_name(self.network_name, "network_name")
        if type(self.published_ports) is not tuple or self.published_ports:
            raise DockerFleetPolygonContractError("host ports are forbidden")
        if self.docker_socket_mounted:
            raise DockerFleetPolygonContractError("Docker socket mount forbidden")
        if isinstance(self.exit_code, bool) or not isinstance(self.exit_code, int):
            raise DockerFleetPolygonContractError("exit_code invalid")
        if isinstance(self.restart_count, bool) or not isinstance(self.restart_count, int) or self.restart_count < 0:
            raise DockerFleetPolygonContractError("restart_count invalid")
        return self
    ...
    def canonical_dict(self) -> dict[str, Any]:
        ...
        value["cap_drop"] = list(self.cap_drop)
        value["security_opt"] = list(self.security_opt)
        value["published_ports"] = list(self.published_ports)
        return value
...
class DockerFleetReconciliationReceipt:
    ...
    def validate(self) -> "DockerFleetReconciliationReceipt":
        if self.schema_version != SCHEMA_VERSION:
            raise DockerFleetPolygonContractError("reconciliation schema mismatch")
        for name in ("plan_digest", "observation_digest", "fleet_result_digest"):
            _sha256(getattr(self, name), name)
        if self.outcome not in RECONCILIATION_OUTCOMES:
            raise DockerFleetPolygonContractError("reconciliation outcome invalid")
        if type(self.mismatches) is not tuple or len(set(self.mismatches)) != len(self.mismatches):
            raise DockerFleetPolygonContractError("mismatches invalid")
        if self.outcome == "MATCHED" and self.mismatches:
            raise DockerFleetPolygonContractError("MATCHED cannot contain mismatches")
        if self.outcome == "MISMATCHED" and not self.mismatches:
            raise DockerFleetPolygonContractError("MISMATCHED requires evidence")
        return self

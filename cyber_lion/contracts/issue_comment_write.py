...
class IssueCommentWriteRequest:
 ...
 def validate(self):
  if self.repository!=REPOSITORY or self.issue_number!=CONTROL_ISSUE: raise IssueCommentWriteContractError("repository/issue outside closed world")
  ...
  expected={"CREATE_COMMENT":{"actions.control-ledger.create","actions.failure-receipt.create","repository-maintenance.receipt.create"},"UPDATE_OWN_CREATED_COMMENT":{"actions.control-ledger.update"}}
  if self.semantic_capability not in expected[self.action]: raise IssueCommentWriteContractError("capability/action mismatch")
...
class CanonicalIssueCommentWriteAdmission:
 ...
 authority_epoch:int; authority_state_version:int; provider_id:str; admission_digest:str=""
 ...
 def validate(self):
  ...
  if self.repository!=REPOSITORY or self.issue_number!=CONTROL_ISSUE or self.action not in ACTIONS or self.semantic_capability not in CAPABILITIES: raise IssueCommentWriteContractError("admission scope invalid")
 ...
 def binds(self,r:IssueCommentWriteRequest):
  ...
  if (self.request_digest,self.repository,self.issue_number,self.action,self.semantic_capability,self.body_digest,self.expected_repository_head)!=(r.request_digest,r.repository,r.issue_number,r.action,r.semantic_capability,body_digest(r.body),r.expected_repository_head): raise IssueCommentWriteContractError("admission/request substitution")

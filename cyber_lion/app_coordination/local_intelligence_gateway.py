...
async function pollSaas(h){if(!h||!h.request_id)return;let deadline=Date.now()+90000;busyEl.textContent='SaaS handoff · oczekiwanie na bieżącą sesję ChatGPT…';while(Date.now()<deadline){await new Promise(r=>setTimeout(r,1000));try{let r=await fetch('/api/saas/requests/'+encodeURIComponent(h.request_id),{cache:'no-store'}),x=await r.json();if(r.ok&&x.status==='RESPONDED'){let model=(x.response_meta_json&&JSON.parse(x.response_meta_json).model_identity)||'ChatGPT';if(h.dual_request_id){let rr=await fetch('/api/dual/response',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({request_id:h.dual_request_id,provider:'CHATGPT_SAAS_SUPERVISOR',response_text:x.response_text||'',transport:(x.response_meta_json&&JSON.parse(x.response_meta_json).transport)||'CHATGPT_SENTINELX_SESSION_MEDIATED'})});if(!rr.ok)throw new Error('dual SaaS receipt '+rr.status);let jr=await fetch('/api/dual/'+encodeURIComponent(h.dual_request_id),{cache:'no-store'}),j=await jr.json();if(!jr.ok)throw new Error(j.error||('dual join '+jr.status));let text=j.answer||('Dual join state: '+j.state);addMsg('assistant',text);history.push({role:'assistant',content:text});history=history.slice(-16);routeEl.textContent='route: DUAL_EVALUATION_JOINED · '+(j.state||'')+' · receipt '+(x.receipt_digest||'').slice(0,12)}else{let label='**SaaS supervisor · '+model+'**\n\n';addMsg('assistant',label+(x.response_text||''));history.push({role:'assistant',content:label+(x.response_text||'')});history=history.slice(-16);routeEl.textContent='route: SAAS_HANDOFF · receipt '+(x.receipt_digest||'').slice(0,12)}busyEl.textContent='';state();return}if(r.ok&&['EXPIRED','SUPERSEDED','REJECTED'].includes(x.status)){let msg=x.status==='SUPERSEDED'?'SaaS handoff pochodzi ze starszej polityki single-pending i został historycznie zastąpiony. Nowy bridge używa kolejki FIFO i nie zastępuje aktywnych pytań. Wyślij pytanie ponownie, jeśli nadal oczekujesz odpowiedzi.':'SaaS handoff zakończony bez odpowiedzi: '+x.status;addMsg('assistant',msg);busyEl.textContent='';return}}catch(e){console.warn(e)}}busyEl.textContent='SaaS handoff nadal oczekuje; możesz ponowić odczyt później.'}
async function go(question){let text=(question??qEl.value).trim();if(!text||sendEl.disabled)return;lastQuestion=text;addMsg('user',text);qEl.value='';sendEl.disabled=true;busyEl.textContent='Routing → evidence → model…';let start=performance.now();try{if(!activeThreadId)await createThread();let resp=await fetch('/api/threads/'+activeThreadId+'/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,output_language:langEl.value})});let x=await resp.json();if(!resp.ok)throw new Error(x.error||('HTTP '+resp.status));lastPayload=x;lastAnswer=x.answer||'';addMsg('assistant',lastAnswer||x.error);history.push({role:'user',content:text},{role:'assistant',content:lastAnswer});history=history.slice(-16);await refreshThreads();routeEl.textContent='route: '+(x.route||'')+' · '+Math.round(performance.now()-start)+' ms';evidenceEl.innerHTML=evidenceHtml(x);debugEl.textContent=JSON.stringify(x,null,2);if(!dbgEl.checked)evidenceEl.classList.remove('hide');if(x.saas_handoff)pollSaas(x.saas_handoff)}catch(e){addMsg('assistant','Błąd: '+e.message)}finally{sendEl.disabled=false;busyEl.textContent='';state()}}
...
function renderMissionList(){missionListEl.innerHTML=missions.map(m=>{let rel=m.state==='SUPERSEDED'&&m.runtime_state?' · '+m.runtime_state:'';return '<button class="mission-item '+(m.mission_id===selectedMissionId?'active':'')+'" data-mid="'+esc(m.mission_id)+'"><b>'+esc(m.title||m.mission_id)+'</b><span>'+esc(m.state)+rel+' · '+Number(m.progress||0).toFixed(1)+'% · '+esc(m.current_phase||'no phase')+'</span><small>'+esc(m.objective||'')+'</small></button>'}).join('');missionListEl.querySelectorAll('[data-mid]').forEach(b=>b.onclick=()=>{selectedMissionId=b.dataset.mid;refreshMissionProcess()})}
...
async function refreshMissionProcess(){if(!selectedMissionId)return;let r=await fetch('/api/missions/'+encodeURIComponent(selectedMissionId)+'/process',{cache:'no-store'});if(!r.ok)return;missionData=await r.json();renderMissionList();let p=missionData.process||{},d=missionData.execution_driver||{};$('missionTitle').textContent=missionData.title||missionData.mission_id;$('missionMeta').textContent=missionData.mission_id+' · '+missionData.state+' · runtime '+missionData.runtime_state+(d.driver_id?' · DRIVER '+d.state+' gen '+d.generation+' · heartbeat '+(d.heartbeat_at||'NONE'):' · DRIVER NOT MATERIALIZED');$('missionObjective').textContent=p.objective||'Cel misji nie został zapisany.';$('missionDescription').textContent=(p.description||'')+(d.driver_id?'\n\nDRIVER: '+d.state+' · phase '+(d.current_phase||'—')+' · gate '+(d.blocking_gate||'—')+' · wait '+(d.waiting_reason||'—')+' · next '+(d.next_action||'—'):'');let has=p.progress!==null&&p.progress!==undefined,pr=has?Number(p.progress):0;$('missionProgressLabel').textContent=has?'Postęp '+pr.toFixed(1)+'%':'Postęp: NOT RECORDED';$('missionPhaseLabel').textContent=p.current_phase?'Aktywna faza: '+p.current_phase:'Brak aktywnej fazy';$('missionProgressBar').style.width=Math.max(0,Math.min(100,pr))+'%';$('missionPhases').innerHTML=(missionData.phases||[]).map(x=>'<div class="phase-card"><div><b>'+esc(x.phase_id)+' · '+esc(x.title)+'</b><span>'+esc(x.status)+'</span></div><div class="mission-progress"><i style="width:'+Math.max(0,Math.min(100,Number(x.progress||0)))+'%"></i></div><small>'+esc(x.detail||'')+'</small></div>').join('')||'<div class="status">Brak planu faz.</div>';let a='';if(missionData.state==='SUPERSEDED'){let succ=String(missionData.runtime_state||'').replace(/^REBOUND_TO:|^SUPERSEDED_BY:/,'');a='<span class="status"><b>Misja historyczna · SUPERSEDED.</b> Następca: '+esc(succ||'UNKNOWN')+'.</span>'}else{a+='<button onclick="missionAction(\'REFRESH\')">Refresh</button><button onclick="missionAction(\'AUDIT\')">Audit</button><button onclick="missionAction(\'VALIDATE\')">Validate</button><button onclick="missionAction(\'RESTART\')">Restart mission</button>';if(['ACTIVE','WAITING','BLOCKED'].includes(d.state))a+='<button onclick="missionAction(\'PAUSE\')">Pause driver</button>';if(['BOOTSTRAP_PAUSED','PAUSED','STOPPED','FAILED'].includes(d.state))a+='<button onclick="missionAction(\'RESUME\')">Resume driver</button>';if(d.state&&!['COMPLETE','STOPPED'].includes(d.state))a+='<button class="danger" onclick="missionAction(\'STOP\')">Stop driver</button>';a+='<span class="status">Material '+Number(missionData.ready||0)+'/'+Number(missionData.materialized||0)+' ready · authority '+esc(missionData.control_authority||'NONE')+'</span>'}$('missionActions').innerHTML=a;renderProtocols();$('controlHealth').textContent='MISSION CONTROL · '+missionData.state+(d.driver_id?' · DRIVER '+d.state:'')}
async function missionAction(action,payload={}){if(!selectedMissionId)return;if(!confirm(action+' mission '+selectedMissionId+'?'))return;let r=await fetch('/api/missions/'+encodeURIComponent(selectedMissionId)+'/action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,payload})}),x=await r.json();if(!r.ok)alert(x.error||'action failed');await refreshMissions()}
...
qEl.addEventListener('keydown',e=>{if(e.key==='Enter'&&e.ctrlKey){e.preventDefault();go()}});async function boot(){await Promise.allSettled([refreshThreads(),refreshMissions(),state()]);if(!threads.length){try{let legacy=JSON.parse(localStorage.getItem('lion_r10_history')||'[]');if(Array.isArray(legacy)&&legacy.length){let r=await fetch('/api/threads/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:legacy.slice(-16)})});if(r.ok){localStorage.removeItem('lion_r10_history');await refreshThreads()}}}catch(e){}}if(threads.length)await openThread(threads[0].thread_id);else await createThread()}boot().catch(e=>{let h=$('controlHealth');if(h)h.textContent='BOOT DEGRADED · '+e.message});setInterval(()=>{Promise.allSettled([state(),refreshMissions()])},5000);
</script></body></html>'''

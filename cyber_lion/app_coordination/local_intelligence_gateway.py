...
async function refreshMissionProcess(){if(!selectedMissionId)return;let r=await fetch('/api/missions/'+encodeURIComponent(selectedMissionId)+'/process',{cache:'no-store'});if(!r.ok)return;missionData=await r.json();renderMissionList();let p=missionData.process||{};$('missionTitle').textContent=missionData.title||missionData.mission_id;$('missionMeta').textContent=missionData.mission_id+' · '+missionData.state+' · runtime '+missionData.runtime_state;$('missionObjective').textContent=p.objective||'Cel misji nie został zapisany.';$('missionDescription').textContent=p.description||'';let pr=Number(p.progress||0);$('missionProgressLabel').textContent='Postęp '+pr.toFixed(1)+'%';$('missionPhaseLabel').textContent=p.current_phase?'Aktywna faza: '+p.current_phase:'Brak aktywnej fazy';$('missionProgressBar').style.width=Math.max(0,Math.min(100,pr))+'%';$('missionPhases').innerHTML=(missionData.phases||[]).map(x=>'<div class="phase-card"><div><b>'+esc(x.phase_id)+' · '+esc(x.title)+'</b><span>'+esc(x.status)+'</span></div><div class="mission-progress"><i style="width:'+Math.max(0,Math.min(100,Number(x.progress||0)))+'%"></i></div><small>'+esc(x.detail||'')+'</small></div>').join('')||'<div class="status">Brak planu faz.</div>';let a='';if(missionData.state==='SUPERSEDED'){let succ=String(missionData.runtime_state||'').replace(/^REBOUND_TO:|^SUPERSEDED_BY:/,'');a='<span class="status"><b>Misja historyczna · SUPERSEDED.</b> Nie jest wykonywana równolegle. Następca: '+esc(succ||'UNKNOWN')+'. Dane pozostają dostępne jako lineage/evidence.</span>'}else if(missionData.control_authority==='BOUNDED_MISSION_CONTROL'){if(missionData.state==='RUNNING')a+='<button onclick="missionAction(\'PAUSE\')">Pause</button><button onclick="missionAction(\'VALIDATE\')">Validate fleet</button>';if(missionData.state==='PAUSED')a+='<button onclick="missionAction(\'RESUME\')">Resume</button>';if(['RUNNING','PAUSED','FAILED'].includes(missionData.state))a+='<button class="danger" onclick="missionAction(\'STOP\')">Stop</button>'}else{let mr=Number(missionData.materialized||0),rr=Number(missionData.ready||0);a=(mr>0||rr>0)?'<span class="status"><b>Material executor aktywny:</b> '+rr+'/'+mr+' ready. Interaktywny adapter akcji tej misji nie jest związany z panelem; wykonanie pozostaje sterowane przez aktywowany LPCL i bounded material plane.</span>':'<span class="status">Brak materialnego executora dla tej misji; stan pozostaje tylko obserwowalny.</span>'}$('missionActions').innerHTML=a;renderProtocols();$('controlHealth').textContent='MISSION CONTROL · '+missionData.state}

...
function renderMissionList(){missionListEl.innerHTML=missions.map(m=>{let rel=m.state==='SUPERSEDED'&&m.runtime_state?' · '+m.runtime_state:'',focus=m.mission_id===missionFocusId?' · FOCUS':'';return '<button class="mission-item '+(m.mission_id===selectedMissionId?'active':'')+'" data-mid="'+esc(m.mission_id)+'"><b>'+esc(m.title||m.mission_id)+'</b><span>'+esc(m.state)+rel+' · '+Number(m.progress||0).toFixed(1)+'% · '+esc(m.current_phase||'no phase')+focus+'</span><small>'+esc(m.objective||'')+'</small></button>'}).join('');missionListEl.querySelectorAll('[data-mid]').forEach(b=>b.onclick=()=>{selectedMissionId=b.dataset.mid;missionPinned=selectedMissionId!==missionFocusId;refreshMissionProcess()})}

...
<section class="control-panel" id="lionBusPanel"><div class="top"><div><div class="k">SHARED OPERATOR MESSAGE PLANE</div><h2>LION BUS · SENTINELX</h2><p class="status">8780 i ten ChatGPT operator uczestniczą w tym samym <code>operator_messages</code> ledgerze przez 8767. Brak browser mediation i brak provider API.</p></div><button type="button" onclick="refreshActiveBus(false)">Odśwież bus</button></div><div id="busCards" class="cards"></div><div id="busStatus" class="status">Wybierz wątek i przypnij misję.</div></section><section class="control-panel"><div class="k">LOCAL COGNITIVE EXECUTOR</div><h2>LION Local Model</h2><p class="status">Proposal-only GPT‑OSS · live Mission Control/repo/web evidence through material drones · authority NONE</p></section>
...
async function operatorPair(){const code=$('operatorPairing').value.trim();try{const body=code?{pairing_code:code}:{};const x=await operatorApi('/api/operator/pair',{method:'POST',headers:{'Content-Type':'application/json','X-LION-CSRF':OPERATOR_CSRF},body:JSON.stringify(body)});$('operatorPairing').value='';$('operatorPairState').textContent=x.paired?'PAIRED · OPERATOR_PRIMARY':'UNPAIRED';await refreshOperator();await refreshActiveBus(false)}catch(e){$('operatorPairState').textContent='PAIR DENIED · '+e.message}}
async function operatorUnpair(){try{await operatorApi('/api/operator/unpair',{method:'POST',headers:{'Content-Type':'application/json','X-LION-CSRF':OPERATOR_CSRF},body:'{}'});if(operatorStream){operatorStream.close();operatorStream=null;operatorStreamMission=null}$('operatorPairState').textContent='UNPAIRED';operatorProjection=null;$('operatorResult').textContent='Operator control: sesja rozłączona';routeEl.textContent='LION BUS · PAIR OPERATOR'}catch(e){$('operatorResult').textContent='UNPAIR DENIED · '+e.message}}
        ...
        def do_POST(self):
            try:
                ...
                if path.startswith('/api/threads/') and path.endswith('/bus'):
                    ...
                    if type(x) is not dict or set(x)!={'content'} or not isinstance(x.get('content'),str) or not x['content'].strip():raise ValueError('bus message schema')
                    ...
                    command={'command_id':'panel-'+uuid.uuid4().hex,'mission_id':context['mission_id'],'action':'MESSAGE','target':context['target'],'payload':{'content':x['content'].strip()[:16000]},'correlation_id':tid}

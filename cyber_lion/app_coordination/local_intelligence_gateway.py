...
<section class="control-panel"><div class="k">HYBRID COGNITIVE PLANE</div><h2>LION Local + SaaS Supervisor</h2><p class="status">LOCAL gpt-oss-20b-MXFP4 proposal-only · REMOTE CHATGPT_SAAS_SUPERVISOR via EXTERNAL_SESSION_MEDIATED · automatic hop not materialized · authority NONE</p></section>
...
async function state(){try{let x=await(await fetch('/api/state',{cache:'no-store'})).json(),m=x.material||{},mc=x.mission_control||{};let rows=[['MODEL',x.model],['GPU',x.gpu||'RTX 5090'],['RAG',x.rag_status],['WEB','AUTO HTTPS'],['REPOS','AUTO READ'],['MISSION',mc.status==='OK'?(mc.focus?.state||'OK'):'UNKNOWN'],['MAT12',(m.healthy??0)+'/12 healthy'],['AUTH',x.authority_effect]];cardsEl.innerHTML=rows.map(z=>'<div class="card"><div class="k">'+z[0]+'</div><div class="v">'+esc(z[1])+'</div></div>').join('')}catch(e){cardsEl.innerHTML='<div class="card">State unavailable</div>'}}
...
class Gateway:
    def __init__(self,repo,rag,rag_sha,release,model_url,model_sha,provider,currentness_provider,git_provider,web=None,content_provider=None,source_provider=None,mission_provider=None,control_provider=None,material_begin=None,material_receipts=None,material_state=None,material_reconcile=None,thread_provider=None,saas_transport='EXTERNAL_SESSION_MEDIATED',saas_model='UNKNOWN'):
        ...
        self.saas_transport=saas_transport;self.saas_model=saas_model
    def state(self):
        ...
        mission_material={'target':(focus or {}).get('material_target'),'materialized':(focus or {}).get('materialized'),'ready':(focus or {}).get('ready'),'mission_id':(focus or {}).get('mission_id')} if focus else {'target':None,'materialized':None,'ready':None,'mission_id':None}
        return {'status':'ok','product':'LION CONTROL LPCL PANEL','system_class':'HYBRID_AI_NATIVE_CONTROL_AND_EXECUTION_ENVIRONMENT','local_model_product':'LION Local Model','model':'gpt-oss-20b-MXFP4','model_sha256':self.model_sha,'gpu':'NVIDIA GeForce RTX 5090 / Vulkan0','system_context_digest':self.ctx.digest,'rag_status':self.rag_status,'rag_release':self.rag.release_id,'rag_sha256':self.rag.sha256,'web_capability':'MEDIATED_PUBLIC_HTTPS_READ_ONLY_AUTO','repository_capability':'MEDIATED_READ_ONLY_AUTO','mission_control':mission,'material':mat,'material_read_plane':mat,'mission_material_plane':mission_material,'saas_capability':'EXTERNAL_SESSION_MEDIATED','saas_supervisor':{'provider':'CHATGPT_SAAS_SUPERVISOR','transport':self.saas_transport,'model':self.saas_model,'state':'AVAILABLE_VIA_EXTERNAL_SESSION' if self.saas_transport=='EXTERNAL_SESSION_MEDIATED' else 'DEGRADED','automatic_hop_materialized':False,'authority_effect':'NONE'},'execution_policy':'HYBRID_EVIDENCE_FIRST_AUTHORITY_BOUND','tool_authority':'NONE','authority_effect':'NONE'}
    def _route(self,message):
        ...
        dual=(('saas' in low or 'chatgpt' in low) and ('lokal' in low or 'local' in low) and any(x in low for x in ('zapytaj','ask','porównaj','porownaj','compare','to samo pytanie','same question')))
        if dual:return 'DUAL_EVALUATION','explicit local plus SaaS comparison requested'
        if re.match(r'^\s*(?:co to|czym jest|what is)\s+lion\s*[?.!]*$',low):return 'SYSTEM_CONTEXT','canonical LION identity from system context'
    ...
    @staticmethod
    def _dual_question(message):
        text=str(message).strip()
        for sep in (':','\n'):
            if sep in text:
                tail=text.rsplit(sep,1)[-1].strip()
                if tail and len(tail)>=3:return tail
        m=re.search(r'(?:pytanie|question)\s*[—-]?\s*(.+)$',text,re.IGNORECASE)
    ...
    def chat(self,message,use_web=False,history=None,output_language='auto'):
        ...
        if route=='DUAL_EVALUATION':
            question=self._dual_question(message)
            local=self.chat(question,use_web=use_web,history=history,output_language=output_language)
            request_id=sha256((self.ctx.digest+'\0'+question).encode('utf-8')).hexdigest()
            handoff={'schema':'LION_SAAS_HANDOFF/1','request_id':request_id,'question':question,'provider':'CHATGPT_SAAS_SUPERVISOR','transport':self.saas_transport,'saas_model':self.saas_model,'status':'AWAITING_EXTERNAL_SESSION_MEDIATION','automatic_hop_materialized':False,'authority_effect':'NONE','system_context_digest':self.ctx.digest}
            answer=('### Model lokalny · gpt-oss-20b-MXFP4\n'+str(local.get('answer') or '')+'\n\n### CHATGPT_SAAS_SUPERVISOR\nŻądanie zostało przygotowane dla kanału '+self.saas_transport+'. Automatyczny hop z lokalnego UI do bieżącej sesji SaaS nie jest jeszcze zmaterializowany, więc LION nie będzie udawał odpowiedzi zdalnego modelu. Handoff ID: `'+request_id[:16]+'`.')
            return {'route':route,'answer':answer,'authority_boundary':False,'rag_sources':local.get('rag_sources',[]),'currentness':local.get('currentness',[]),'web_sources':local.get('web_sources',[]),'web_fetches':local.get('web_fetches',[]),'source_evidence':local.get('source_evidence',[]),'mission_control':local.get('mission_control',{}),'tool_calls':local.get('tool_calls',[]),'material_receipts':local.get('material_receipts',[]),'response_language':output_language,'local_evaluation':{'model':'gpt-oss-20b-MXFP4','answer':local.get('answer'),'route':local.get('route')},'saas_handoff':handoff}

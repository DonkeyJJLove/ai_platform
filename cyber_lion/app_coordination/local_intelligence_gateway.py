...
<section class="control-panel" id="operatorPanel"><div class="top"><div><div class="k">HUMAN OPERATOR CONTROL</div><h2>OPERATOR_PRIMARY</h2><p class="status">Sterowanie misją · rozmowa z modelami odbywa się wyłącznie w aktywnym wątku LION BUS</p></div><button type="button" onclick="refreshOperator()">Odśwież operatora</button></div><div class="row operator-session-row"><button id="operatorPairButton" type="button" class="primary" onclick="operatorPair()">Aktywuj sterowanie operatorem</button><button id="operatorUnpairButton" type="button" onclick="operatorUnpair()">Rozłącz operatora</button><button type="button" onclick="focusBusComposer()">Przejdź do rozmowy</button><span id="operatorPairState" class="status">UNPAIRED · lokalny handshake</span></div><div id="operatorCards" class="cards"></div><div class="row"><label>Operacja <select id="operatorMode" data-operator-control disabled><option value="AMEND_CONTEXT">Korekta kontekstu</option><option value="AMEND_PLAN">Korekta planu</option></select></label><label>Adresat <select id="operatorTarget" data-operator-control disabled></select></label></div><textarea id="operatorText" data-operator-control disabled class="lpcl-box" style="min-height:90px" placeholder="Korekta kontekstu lub planu dla wybranego adresata…"></textarea><div class="row"><button type="button" class="primary" data-operator-control disabled onclick="operatorSend()">Zastosuj korektę</button><button type="button" data-operator-control disabled onclick="operatorControl('PAUSE_SCOPE')">Wstrzymaj</button><button type="button" class="danger" data-operator-control disabled onclick="operatorControl('STOP_SCOPE')">Zatrzymaj</button><button type="button" data-operator-control disabled onclick="operatorControl('TAKE_CONTROL')">Przejmij sterowanie</button><button type="button" data-operator-control disabled onclick="operatorControl('RELEASE_CONTROL')">Oddaj sterowanie</button><button type="button" data-operator-control disabled onclick="operatorControl('RESUME_SCOPE',{latch:'ALL'})">Wznów</button></div><div id="operatorResult" class="status">Operator control: oczekiwanie na stan.</div><h3>Zdarzenia operatora</h3><div id="operatorFeed" class="protocol-feed"></div></section>
...
function renderThreadContext(){const c=activeThreadContext||{},selected=selectedMissionId||missionFocusId,mission=missions.find(m=>m.mission_id===(c.mission_id||selected));const h=$('threadContextHeader'),hint=$('bindingHint'),bind=$('bindMissionButton'),unbind=$('unbindMissionButton');if(h)h.textContent='LION BUS · THREAD '+String(activeThreadId||'—').slice(0,12)+' · '+(c.binding_state||'MISSION_UNBOUND')+' · MISSION '+(c.mission_id||'—')+' · REV '+(c.binding_revision??'—');if(hint)hint.textContent=c.binding_state==='MISSION_BOUND'?'Misja: '+(mission?.title||c.mission_id):selected?'Wybrana misja: '+(mission?.title||selected):'Wybierz misję po lewej';if(bind){bind.disabled=!selected;bind.textContent=c.binding_state==='MISSION_BOUND'&&c.mission_id===selected?'Powiązano':'Powiąż z wybraną misją'}if(unbind)unbind.disabled=c.binding_state!=='MISSION_BOUND'}
...
function focusBusComposer(){qEl.focus();qEl.scrollIntoView({behavior:'smooth',block:'center'})}
async function operatorSend(){const mode=$('operatorMode').value,text=$('operatorText').value.trim(),target=$('operatorTarget').value;if(!text)return;if(mode==='MESSAGE'){qEl.value=text;$('operatorText').value='';await go(text);return}try{let payload={content:text};await operatorSubmit(mode,payload,target);$('operatorText').value=''}catch(e){$('operatorResult').textContent='DENIED · '+e.message}}
        ...
        def do_GET(self):
            ...
            if path.startswith('/api/threads/') and path.endswith('/bus'):
                ...
                try:
                    ...
                    return self.out({'thread_id':tid,'context':context,'messages':messages,'message_deliveries':deliveries,'thread_mission_ids':thread_state.get('mission_ids') or [],'suppressed_response_ids':thread_state.get('suppressed_response_ids') or [],'control':state.get('control'),'operator':state.get('operator'),'operator_proxy':state.get('operator_proxy'),'authority_effect':'NONE'})
                except PermissionError as e:return self.out({'error':str(e)},403)

...
.phase-liveness{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:8px;padding:7px 9px;border:1px solid #314956;border-radius:7px;background:#0b171e;font:600 10px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace}.phase-life-dot{width:8px;height:8px;border-radius:50%;background:#647681;box-shadow:0 0 0 2px #263943}.phase-current{border:1px solid #5ba8c2;border-radius:999px;padding:1px 6px;color:#bfeeff;background:#12313f;letter-spacing:.06em}.phase-life-label{letter-spacing:.04em}.phase-life-detail{font-weight:500;color:#9fb5c2;overflow-wrap:anywhere}.phase-life-executing{border-color:#2f765b;background:#0d241b}.phase-life-executing .phase-life-dot{background:#60dea0;box-shadow:0 0 0 2px #214c3b;animation:phasePulse .45s ease-out 1}.phase-life-waiting-local,.phase-life-waiting-evidence,.phase-life-waiting{border-color:#7d6a33;background:#221d0e}.phase-life-waiting-local .phase-life-dot,.phase-life-waiting-evidence .phase-life-dot,.phase-life-waiting .phase-life-dot{background:#e5bf58;box-shadow:0 0 0 2px #51451f}.phase-life-ready-to-advance{border-color:#398ca4;background:#0d2630}.phase-life-ready-to-advance .phase-life-dot{background:#6bd5f0;box-shadow:0 0 0 2px #25596a}.phase-life-blocked,.phase-life-stopped{border-color:#984a5c;background:#2a1118}.phase-life-blocked .phase-life-dot,.phase-life-stopped .phase-life-dot{background:#ff8298;box-shadow:0 0 0 2px #63313c}.phase-life-paused{border-color:#786b46;background:#211d12}.phase-life-complete{border-color:#34785a;background:#0d2118}.phase-life-complete .phase-life-dot{background:#65d89c;box-shadow:0 0 0 2px #24543d}.phase-life-pending{opacity:.72}.phase-life-pending .phase-life-dot{background:#65737d}.semantic-card:has(.phase-life-ready-to-advance){border-color:#4ca4bd;box-shadow:0 0 0 1px #4ca4bd33 inset}@keyframes phasePulse{0%{box-shadow:0 0 0 0 rgba(96,222,160,.55)}70%{box-shadow:0 0 0 7px rgba(96,222,160,0)}100%{box-shadow:0 0 0 0 rgba(96,222,160,0)}}

.phase-curriculum{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:6px;padding:6px 9px;border:1px solid #344b67;border-radius:7px;background:#101927;color:#b9cbea;font:600 9px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace}.phase-curriculum-tag{border:1px solid #5f78a5;border-radius:999px;padding:1px 6px;color:#d9e7ff;background:#1a2841;letter-spacing:.07em}.phase-curriculum-missing{color:#ffd27a}
...
async function state(indicatorsOnly=false){
  ...
  try{
    ...
    if(!indicatorsOnly)renderModelCalls(modelCalls);
...
qEl.addEventListener('keydown',e=>{if(e.key==='Enter'&&e.ctrlKey){e.preventDefault();go()}});async function boot(){await Promise.allSettled([refreshThreads(),refreshMissions(),state()]);await ensureOperatorSession();await refreshOperator();if(!threads.length){try{let legacy=JSON.parse(localStorage.getItem('lion_r10_history')||'[]');if(Array.isArray(legacy)&&legacy.length){let r=await fetch('/api/threads/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:legacy.slice(-16)})});if(r.ok){localStorage.removeItem('lion_r10_history');await refreshThreads()}}}catch(e){}}if(threads.length)await openThread(threads[0].thread_id);else await createThread();await state()}boot().catch(e=>{let h=$('controlHealth');if(h)h.textContent='BOOT DEGRADED · '+e.message});setInterval(()=>{Promise.allSettled([state(true),refreshMissionProcess()])},5000);setInterval(()=>{if(Date.now()-missionLastStructuralRefreshAt>=30000)Promise.allSettled([state(false),refreshMissions(),refreshOperator()])},30000);setInterval(()=>{void refreshActiveBus(true)},1500);

...
async function pollSaas(h){
 ...
 entry.promise=(async()=>{
  handoffBusy(threadId,'SaaS · oczekuje na operatora; brak automatycznej wysyłki');
  ...
  while(Date.now()<deadline&&!controller.signal.aborted){
   ...
   try{
    ...
    if(x.status==='RESPONDED'){
     ...
     if(controller.signal.aborted)return;
     const saved=await persistSaasAssistant(threadId,key,text,{model_identity:model,transport,receipt_digest:x.receipt_digest||null,dual_request_id:h.dual_request_id||null});
     if(!controller.signal.aborted&&saved.inserted&&threadId===activeThreadId){addMsg('assistant',text);history.push({role:'assistant',content:text});history=history.slice(-16);routeEl.textContent='SaaS · odpowiedź odebrana'}
     await refreshThreads();return x;
    }
    if(['CANCELLED','EXPIRED','SUPERSEDED','REJECTED'].includes(x.status))return x;
    handoffBusy(threadId,'SaaS · oczekuje na operatora; nie wysłano automatycznie');
...
async function activateLpcl(){if(!lpclRegistered)return alert('Najpierw zwaliduj i zarejestruj dokładny LPCL.');if(!confirm('Autoryzować dokładny LPCL digest '+lpclRegistered.lpcl_digest.slice(0,16)+'?'))return;let r=await fetch('/api/lpcl/activate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(lpclRegistered)}),x=await r.json();if(!r.ok)return alert(x.error||'activation failed');$('lpclStatus').textContent=(x.state||'UNKNOWN')+' · runtime '+(x.runtime_state||'UNKNOWN');$('lpclPreview').textContent=JSON.stringify({mission_id:x.mission_id,state:x.state,runtime_state:x.runtime_state,error:x.last_error||null,driver:x.execution_driver||null},null,2);selectedMissionId=lpclRegistered.mission_id;await refreshMissions()}

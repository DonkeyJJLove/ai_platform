        ...
        def do_GET(self):
            ...
            if path.startswith('/api/threads/') and path.endswith('/bus'):
                ...
                try:
                    ...
                    state=self._operator('state',{'mission_id':context['mission_id'],'session_token':browser_session.get('gateway_session')});messages=[m for m in (state.get('messages') or []) if m.get('correlation_id')==tid and not (m.get('kind')=='RESPONSE' and m.get('conversation_valid') is False)];messages.sort(key=lambda m:(m.get('created_at') or '',m.get('message_id') or ''));ids={m.get('message_id') for m in messages};deliveries=[d for d in (state.get('message_deliveries') or []) if d.get('message_id') in ids]

...
def build_lion_context(root):
    ...
    text=(f"PROJECT=LION_EVOLUSION\nARCHITECTURE_EPOCH=1.4\nMATERIAL_EPOCH=R10\nLION_SYSTEM_CLASS=HYBRID_AI_NATIVE_CONTROL_AND_EXECUTION_ENVIRONMENT\nLION_NAME=LION\nLION_ACRONYM_EXPANSION=UNDEFINED_NEVER_INVENT\nRAG_RELEASE={release}\n"
          "COGNITIVE_PLANE: LION is obligatorily hybrid. LOCAL=gpt-oss-20b-MXFP4 as proposal-only cognitive executor. REMOTE=CHATGPT_SAAS_SUPERVISOR as supervisory reasoning plane only when a current Mission Control transport/session binding is evidenced. Transport identity and automatic-hop state are live currentness and must never be inherited from historical configuration.\n"

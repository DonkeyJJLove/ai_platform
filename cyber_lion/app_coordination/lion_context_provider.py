...
def build_lion_context(root):
    ...
    text=(f"PROJECT=LION_EVOLUSION\nARCHITECTURE_EPOCH=1.4\nMATERIAL_EPOCH=R10\nMODEL_ROLE=PROPOSAL_ONLY\nRAG_RELEASE={release}\n"
          "AUTHORIZATION: LPCL generation is not authority. Explicit user launch/run of the exact LPCL activates only its bounded scope. A successor identity outside that scope requires a new LPCL and a new user launch.\n"
          "INVARIANTS: MODEL_OUTPUT_NE_AUTHORITY; TOOL_AVAILABILITY_NE_AUTHORITY; RAG_NE_LIVE_TRUTH; LOGICAL_DRONE_NE_MATERIAL_EXECUTOR; PR_NE_MERGE_AUTHORITY; MERGE_NE_RUNTIME_AUTHORITY.\n"
          "EPISTEMIC: model output is not system truth; RAG is versioned knowledge, not live truth; live claims require currentness readback.\n"
          ...
          "EXECUTION: logical drone count is not material executor count; PR authority is separate from merge authority; merge is separate from runtime authority.\n"
          "MATERIAL_FLEET: count=12; MAT01=LOCAL_REPOSITORY_CURRENTNESS; MAT02=LOCAL_REPOSITORY_CONTENT; MAT03=LOCAL_CLONE_INVENTORY; MAT04=FEDERATION_CURRENTNESS; MAT05=PUBLIC_WEB_SEARCH; MAT06=PUBLIC_WEB_FETCH; MAT07=WEB_SECURITY_FALSIFIER; MAT08=SOURCE_PROVENANCE_VALIDATOR; MAT09=MODEL_GPU_OBSERVER; MAT10=RESULT_VALIDATOR; MAT11=MATERIAL_COORDINATOR; MAT12=FINAL_RECONCILER; authority=NONE; material drone is not physical failure domain.\n"

...
class ActionsControlLedgerBoundary:
    """Mandatory issue-comment PEP for the Actions bridge control ledger."""

    _process_lock = threading.RLock()

    def __init__(self, repository: str, token: str, *, observer: ActionsControlLedgerObserver | None = None) -> None:
        if not _REPOSITORY_RE.fullmatch(repository):
            raise ActionsControlLedgerError("repository identity invalid")
        if not isinstance(token, str) or not token:
            raise ActionsControlLedgerError("GitHub token unavailable")
        ...
        if type(self._observer) is not ActionsControlLedgerObserver:
            raise ActionsControlLedgerError("exact independent observer component required")
    ...
    def _headers(self) -> dict[str, str]:
        return {
            ...
            "User-Agent": "lion-actions-control-ledger-boundary/1",
        }

    @staticmethod
    def _validate_create_body(body: str) -> tuple[str | None, str | None]:
        if not isinstance(body, str) or not body.startswith(_ALLOWED_CREATE_PREFIXES):
            raise ActionsControlLedgerError("control-ledger create body not allowlisted")
        _body_digest(body)
        fields = _fields(body)
        request_id = fields.get("request_id")
        replay_key = fields.get("replay_key")
        if request_id is not None and _REQUEST_ID_RE.fullmatch(request_id) is None:
            raise ActionsControlLedgerError("request id invalid")
        if replay_key is not None and _HEX64_RE.fullmatch(replay_key) is None:
            raise ActionsControlLedgerError("replay key invalid")
        return request_id, replay_key

    @staticmethod
    def _validate_update_body(body: str) -> None:
        if not isinstance(body, str) or not body.startswith(_ALLOWED_UPDATE_PREFIXES):
            raise ActionsControlLedgerError("control-ledger update body not allowlisted")
        _body_digest(body)

    def _post(self, body: str) -> int:
        payload = _canonical({"body": body})
        req = urllib.request.Request(
            f"https://api.github.com/repos/{self.repository}/issues/{CONTROL_ISSUE}/comments",
            data=payload,
            method="POST",
        ...
        try:
            with urllib.request.build_opener(_NoRedirect()).open(req, timeout=20) as response:
                if response.status != 201:
                    raise ActionsControlLedgerError("control-ledger create rejected")
                raw = response.read(_MAX_RESPONSE_BYTES + 1)
        except urllib.error.URLError as exc:
            raise ActionsControlLedgerError("control-ledger create failed") from exc
        if len(raw) > _MAX_RESPONSE_BYTES:
            raise ActionsControlLedgerError("control-ledger create response oversized")
        try:
            value = json.loads(raw)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise ActionsControlLedgerError("control-ledger create response malformed") from exc
        if not isinstance(value, dict) or not isinstance(value.get("id"), int) or value["id"] <= 0:
            raise ActionsControlLedgerError("control-ledger create identity missing")
        return int(value["id"])

    def _patch(self, comment_id: int, body: str) -> None:
        payload = _canonical({"body": body})
        req = urllib.request.Request(
            f"https://api.github.com/repos/{self.repository}/issues/comments/{comment_id}",
            data=payload,
            method="PATCH",
            headers=self._headers(),
        )
        try:
            with urllib.request.build_opener(_NoRedirect()).open(req, timeout=20) as response:
                if response.status != 200:
                    raise ActionsControlLedgerError("control-ledger update rejected")
                response.read(_MAX_RESPONSE_BYTES + 1)
        except urllib.error.URLError as exc:
            raise ActionsControlLedgerError("control-ledger update failed") from exc

    def create(self, issue_number: int, body: str) -> int:
        if issue_number != CONTROL_ISSUE:
            raise ActionsControlLedgerError("control issue substitution denied")
        request_id, replay_key = self._validate_create_body(body)
        with self._process_lock:
            # Durable GitHub comment ledger + workflow-level serial concurrency is the
            # cross-process replay domain.  Recheck immediately before the first POST.
            if request_id is not None or replay_key is not None:
                for item in self._observer.comments():
                    ...
                    if request_id is not None and fields.get("request_id") == request_id:
                        raise ActionsControlLedgerError("request-id replay denied")
                    if replay_key is not None and fields.get("replay_key") == replay_key:
                        raise ActionsControlLedgerError("replay-key replay denied")
            comment_id = self._post(body)
            observation = self._observer.comment(comment_id, body)
            if not observation.observed:
                raise ActionsControlLedgerError("control-ledger create read-back mismatch")
            self._created_ids.add(comment_id)
            return comment_id

    def update(self, comment_id: int, body: str) -> None:
        self._validate_update_body(body)
        if comment_id not in self._created_ids:
            raise ActionsControlLedgerError("update target was not created by this boundary instance")
        with self._process_lock:
            self._patch(comment_id, body)
            observation = self._observer.comment(comment_id, body)
            if not observation.observed:
                raise ActionsControlLedgerError("control-ledger update read-back mismatch")

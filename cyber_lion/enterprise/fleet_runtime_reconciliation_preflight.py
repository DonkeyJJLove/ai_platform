...
def _execution_receipt_state(path: Path) -> tuple[bool | None, bool]:
    """Return (present, stable); never creates or modifies the target."""
    try:
        before_present = _file_present(path, "runtime execution receipt")
    except RuntimeReconciliationPreflightError:
        return None, False
    if before_present:
        try:
            ...
            return True, False
    try:
        after_present = _file_present(path, "runtime execution receipt")
    except RuntimeReconciliationPreflightError:
        return before_present, False
...
def _bounded_reconciliation_state(path: Path, repository: str) -> dict[str, Any]:
    """One query-only reconciliation fingerprint used as outer race guard."""
    conn = _ro(path)
    try:
        ...
        if head is None:
            report_rows = conn.execute(
                ...
                "FROM reconciliation_report WHERE repository=? ORDER BY report_digest,report_id",
                (repository,),
            ).fetchall()
            receipt_rows = conn.execute(
                ...
                "FROM convergence_receipt WHERE repository=? ORDER BY receipt_digest,receipt_id",
                (repository,),
            ).fetchall()
        else:
            ...
            report_rows = conn.execute(
                "SELECT report_digest,report_id,repository,inventory_id,inventory_revision,inventory_digest,"
                "closure_preconditions_digest,default_head_sha,disposition,observed_at "
                "FROM reconciliation_report WHERE repository=? AND inventory_digest=? "
                "ORDER BY report_digest,report_id",
                (repository, digest),
            ).fetchall()
            receipt_rows = conn.execute(
                "SELECT receipt_digest,receipt_id,report_digest,repository,inventory_id,inventory_revision,"
                "inventory_digest,closure_preconditions_digest,default_head_sha,issued_at,purpose,consumed "
                "FROM convergence_receipt WHERE repository=? AND inventory_digest=? "
                "ORDER BY receipt_digest,receipt_id",
                (repository, digest),
            ).fetchall()
...
def _structural_reconciliation_conflict(
    ...
    if report_count == 1:
        if not isinstance(canonical_report, dict):
            return True
        if canonical_report.get("report_digest") != reports[0].get("report_digest"):
            return True
        if not canonical.get("report_bound"):
            return True
    ...
    if receipt_count == 1:
        if not isinstance(canonical_receipt, dict):
            return True
        if canonical_receipt.get("receipt_digest") != receipts[0].get("receipt_digest"):
            return True
...
def observe_runtime_reconciliation_preflight(
    ...
    *,
    ...
    observed_names = ("status", "coordination", "reconciliation", "trust", "inventory", "execution_receipt")
    required_names = ("status", "coordination", "reconciliation", "trust", "inventory")
    presence_before = _presence_map(paths, observed_names)
    ...
    inventory = None
    trust = None
    trust_read_conflict = False
    inventory_read_conflict = False
    ...
    if presence_before.get("inventory") is True and trust_valid:
        try:
            ...
            RuntimeReconciliationIngestionError,
            RuntimeReconciliationPreflightError,
            ValueError,
        ):
    ...
    reconciliation_race = False
    head: dict[str, Any] | None = None
    canonical_reconciliation: dict[str, Any] | None = None
    before_reconciliation: dict[str, Any] | None = None
    after_reconciliation: dict[str, Any] | None = None
    ...
    try:
        ...
        if presence_before.get("reconciliation") is True:
            ...
            if isinstance(before_head, dict):
                canonical_reconciliation = _read_reconciliation(
                    str(paths["reconciliation"]),
                    config.repository,
                    str(before_head.get("default_head_sha")),
                )
            ...
            if canonical_reconciliation is not None and isinstance(before_head, dict):
                reconciliation_race = reconciliation_race or not _head_equal(
                    canonical_reconciliation.get("head"),
                    before_head,
                )
            ...
            if reconciliation_ok:
                ...
                structural_conflict = _structural_reconciliation_conflict(
    ...
    presence_after = _presence_map(paths, observed_names)
    presence_error = any(value is None for value in presence_before.values()) or any(
        value is None for value in presence_after.values()
    )
    presence_race = any(presence_before.get(name) != presence_after.get(name) for name in observed_names)
    required_sources_present = all(
        presence_before.get(name) is True and presence_after.get(name) is True
    ...
    source_conflict = bool(
        presence_error
        or presence_race
        or not execution_stable
        or presence_before.get("trust") is not True
        or not trust_valid
        or trust_read_conflict
        or presence_before.get("status") is not True
        or not status_ok
        or presence_before.get("coordination") is not True
        or not coordination_ok
        or presence_before.get("reconciliation") is not True
        or not reconciliation_ok
        or reconciliation_race
    ...
    runtime_source_healthy = bool(
        ...
        and not presence_error
        and not presence_race

...
def _canonical_record_json(record):
    p=asdict(record)
    for k in ("candidate_scope","resource_scope"):
        if k in p:p[k]=list(p[k])
    return json.dumps(p,sort_keys=True,separators=(",",":"),ensure_ascii=False)
def _decode_record(raw,cls):
    ...
    if not isinstance(p,dict) or set(p)!=set(cls.__dataclass_fields__): raise PersistentAuthorityStateError("issuance record is noncanonical")
    for k in ("candidate_scope","resource_scope"):
        if k in p and type(p[k]) is list:p[k]=tuple(p[k])
...
class PersistentBuilderProcessLaunchIntent:
    ...
    def validate(self):
        ...
        if not self.launch_request_id.startswith("bplr:") or self.authority_store_origin_id!=f"aso:{self.authority_store_origin_digest}":raise PersistentAuthorityStateError("launch intent semantics invalid")
        return self
    def canonical_json(self):self.validate();return _canonical_record_json(self)
    @classmethod
    def from_json(cls,v):return _decode_record(v,cls)
    ...
    def from_request(cls,request,*,authority_store_origin,prepared_at):
        ...
        request.validate();authority_store_origin.validate();p={k:v for k,v in asdict(request).items() if k not in {"authority_effect","execution_effect","repository_ref_effect","external_effect","schema_version"}}
        return cls(**p,authority_store_origin_id=authority_store_origin.origin_id,authority_store_origin_digest=authority_store_origin.origin_digest,prepared_at=prepared_at).validate()
@dataclass(frozen=True)
class PersistentBuilderProcessHeldMaterialization:
    launch_id:str;launch_request_id:str;launch_request_digest:str;launch_replay_digest:str;provider_id:str;provider_identity_digest:str;provider_implementation_digest:str;provider_attestation_digest:str;runtime_instance_identity:str;execution_environment_id:str;process_handle_reference:str;process_identity_token:str;held_identity_digest:str;execution_gate_id:str;execution_gate_digest:str;gate_state:str;gate_attestation_digest:str;builder_entrypoint_digest:str;state:str;prepared_at:str;observed_at:str;authority_store_origin_id:str;authority_store_origin_digest:str
    def validate(self):
        for n in ("launch_id","launch_request_id","provider_id","runtime_instance_identity","execution_environment_id","process_handle_reference","process_identity_token","execution_gate_id","gate_state","state","prepared_at","observed_at","authority_store_origin_id"):_text(getattr(self,n),name=n,limit=2048)
        for n in ("launch_request_digest","launch_replay_digest","provider_identity_digest","provider_implementation_digest","provider_attestation_digest","held_identity_digest","execution_gate_digest","gate_attestation_digest","builder_entrypoint_digest","authority_store_origin_digest"):_sha256(getattr(self,n),name=n)
        if self.process_handle_reference.isdigit() or self.state!="HELD_NOT_EXECUTING_BUILDER" or self.gate_state!="CLOSED" or self.authority_store_origin_id!=f"aso:{self.authority_store_origin_digest}":raise PersistentAuthorityStateError("held materialization semantics invalid")
        return self
    def canonical_json(self):self.validate();return _canonical_record_json(self)
    @classmethod
    def from_json(cls,v):return _decode_record(v,cls)
    ...
    def from_identity(cls,identity,request,descriptor,gate,*,authority_store_origin,prepared_at,observed_at):
        from cyber_lion.contracts.builder_process_launch import BuilderExecutionGateEvidence,BuilderProcessIdentity,BuilderProcessLaunchRequest,BuilderProcessRuntimeProviderDescriptor,HELD_STATE,GATE_CLOSED
        if type(identity) is not BuilderProcessIdentity or type(request) is not BuilderProcessLaunchRequest or type(descriptor) is not BuilderProcessRuntimeProviderDescriptor or type(gate) is not BuilderExecutionGateEvidence:raise PersistentAuthorityStateError("exact held materialization inputs required")
        identity.validate();request.validate();descriptor.validate();gate.validate();authority_store_origin.validate()
        if identity.state!=HELD_STATE or identity.identity_digest!=identity.compute_digest() or gate.gate_state!=GATE_CLOSED or gate.execution_gate_digest!=gate.compute_digest():raise PersistentAuthorityStateError("sealed HELD/gate evidence required")
        if (identity.builder_subject_id,identity.builder_instance_id,identity.process_profile_id,identity.process_profile_digest,identity.launch_policy_digest,identity.runtime_provider_id,identity.runtime_provider_identity_digest,identity.runtime_instance_identity)!=(request.builder_subject_id,request.builder_instance_id,request.process_profile_id,request.process_profile_digest,request.launch_policy_digest,descriptor.provider_id,descriptor.provider_identity_digest,descriptor.runtime_instance_identity):raise PersistentAuthorityStateError("held identity request/provider binding mismatch")
        if (gate.launch_id,gate.runtime_instance_identity,gate.execution_environment_id,gate.execution_gate_id)!=(identity.launch_id,descriptor.runtime_instance_identity,identity.execution_environment_id,identity.execution_gate_id):raise PersistentAuthorityStateError("held gate identity binding mismatch")
        return cls(identity.launch_id,request.launch_request_id,request.launch_request_digest,request.launch_replay_digest,descriptor.provider_id,descriptor.provider_identity_digest,descriptor.provider_implementation_digest,descriptor.provider_attestation_digest,descriptor.runtime_instance_identity,identity.execution_environment_id,identity.process_handle_reference,identity.process_identity_token,identity.identity_digest,gate.execution_gate_id,gate.execution_gate_digest,gate.gate_state,gate.gate_attestation_digest,gate.builder_entrypoint_digest,identity.state,prepared_at,observed_at,authority_store_origin.origin_id,authority_store_origin.origin_digest).validate()
@dataclass(frozen=True)
class PersistentBuilderProcessLaunchReceipt:
    launch_receipt_id:str;launch_receipt_digest:str;launch_request_id:str;launch_request_digest:str;launch_replay_digest:str;source_builder_start_admission_id:str;source_builder_start_admission_digest:str;repository:str;baseline_master_sha:str;baseline_master_tree_sha:str;authority_digest_at_launch:str;builder_subject_digest_at_launch:str;process_profile_id:str;process_profile_digest:str;launch_policy_digest:str;runtime_provider_id:str;runtime_provider_identity_digest:str;runtime_provider_implementation_digest:str;runtime_provider_attestation_digest:str;runtime_instance_identity:str;launch_id:str;execution_environment_id:str;process_handle_reference:str;process_identity_token:str;process_identity_digest:str;execution_gate_id:str;execution_gate_closed_digest:str;execution_gate_opened_digest:str;builder_entrypoint_digest:str;launch_started_at:str;launch_observed_at:str;effect_class:str;effect_state:str;gate_transition:str;authority_store_origin_id:str;authority_store_origin_digest:str
    def validate(self):
        for n in ("launch_receipt_id","launch_request_id","source_builder_start_admission_id","repository","process_profile_id","runtime_provider_id","runtime_instance_identity","launch_id","execution_environment_id","process_handle_reference","process_identity_token","execution_gate_id","launch_started_at","launch_observed_at","effect_class","effect_state","gate_transition","authority_store_origin_id"):_text(getattr(self,n),name=n,limit=2048)
        for n in ("launch_receipt_digest","launch_request_digest","launch_replay_digest","source_builder_start_admission_digest","authority_digest_at_launch","builder_subject_digest_at_launch","process_profile_digest","launch_policy_digest","runtime_provider_identity_digest","runtime_provider_implementation_digest","runtime_provider_attestation_digest","process_identity_digest","execution_gate_closed_digest","execution_gate_opened_digest","builder_entrypoint_digest","authority_store_origin_digest"):_sha256(getattr(self,n),name=n)
        _sha40(self.baseline_master_sha,name="baseline_master_sha");_sha40(self.baseline_master_tree_sha,name="baseline_master_tree_sha")
        if self.process_handle_reference.isdigit() or self.effect_class!="BUILDER_PROCESS_START" or self.effect_state!="STARTED_OBSERVED" or self.gate_transition!="CLOSED_TO_OPENED_ONCE" or self.authority_store_origin_id!=f"aso:{self.authority_store_origin_digest}":raise PersistentAuthorityStateError("launch receipt semantics invalid")
        return self
    def canonical_json(self):self.validate();return _canonical_record_json(self)
    @classmethod
    def from_json(cls,v):return _decode_record(v,cls)
    ...
    def from_receipt(cls,receipt,*,authority_store_origin):
        ...
        receipt.validate();authority_store_origin.validate();p={k:v for k,v in asdict(receipt).items() if k not in {"authority_effect","execution_effect","repository_ref_effect","external_effect","schema_version"}}
        return cls(**p,authority_store_origin_id=authority_store_origin.origin_id,authority_store_origin_digest=authority_store_origin.origin_digest).validate()
...
class SQLiteAuthorityStateStore:
    ...
    def current_epoch(self,context):
        ...
        with self._connect() as c:r=c.execute("SELECT epoch,revoked_json,version FROM authority_epoch_state WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=?",context).fetchone()
        if r is None:raise PersistentAuthorityStateError("authority context is not bootstrapped")
        return PersistentEpochSnapshot(*context,int(r[0]),tuple(json.loads(r[1])),int(r[2]))
    def advance_epoch(self,context,*,epoch,revoked_grant_ids):
        context=self._context(context);revoked=self._revoked_json(revoked_grant_ids);candidate=set(json.loads(revoked))
        with self._lock,self._connect() as c:
            c.execute("BEGIN IMMEDIATE");r=c.execute("SELECT epoch,revoked_json,version FROM authority_epoch_state WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=?",context).fetchone()
            if r is None:c.execute("ROLLBACK");raise PersistentAuthorityStateError("authority context is not bootstrapped")
            if epoch<int(r[0]) or (epoch==int(r[0]) and not set(json.loads(r[1])).issubset(candidate)):c.execute("ROLLBACK");raise PersistentAuthorityStateError("authority epoch/revocation cannot roll back")
            c.execute("UPDATE authority_epoch_state SET epoch=?,revoked_json=?,version=? WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=?",(epoch,revoked,int(r[2])+1,*context));c.execute("COMMIT")
        return self.current_epoch(context)
    def register_root(self,context,*,epoch,root_grant_id,root_grant_digest):
        ...
        with self._lock,self._connect() as c:
            try:c.execute("BEGIN IMMEDIATE");r=c.execute("SELECT epoch FROM authority_epoch_state WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=?",context).fetchone();
            ...
            if r is None or int(r[0])!=epoch:c.execute("ROLLBACK");raise PersistentAuthorityStateError("root anchor must bind current epoch")
            try:c.execute("INSERT INTO authority_root_anchor VALUES(?,?,?,?,?,?,?)",(*context,epoch,root_grant_id,root_grant_digest));c.execute("COMMIT")
            except sqlite3.IntegrityError as exc:c.execute("ROLLBACK");raise PersistentAuthorityStateError("root anchor already exists") from exc
        return PersistentRootAnchor(*context,epoch,root_grant_id,root_grant_digest)
    def resolve_root(self,context,*,epoch):
        context=self._context(context)
        with self._connect() as c:r=c.execute("SELECT root_grant_id,root_grant_digest FROM authority_root_anchor WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=? AND epoch=?",(*context,epoch)).fetchone()
        if r is None:raise PersistentAuthorityStateError("root anchor is missing")
        return PersistentRootAnchor(*context,epoch,r[0],r[1])
    ...
    def _req_origin(self,r,label):
        r.validate();o=self.resolve_authority_store_origin()
        if (r.authority_store_origin_id,r.authority_store_origin_digest)!=(o.origin_id,o.origin_digest):raise PersistentAuthorityStateError(f"{label} origin mismatch")
    def record_builder_entry_issuance(self,r):
        ...
        self._req_origin(r,"builder entry issuance")
        with self._lock,self._connect() as c:
    ...
    def resolve_builder_entry_issuance(self,i):
        ...
        self._req_origin(r,"builder entry issuance");return r
    def record_builder_invocation_issuance(self,r):
        ...
        self._req_origin(r,"builder invocation issuance")
        with self._lock,self._connect() as c:
    ...
    def resolve_builder_invocation_issuance(self,i):
        ...
        self._req_origin(r,"builder invocation issuance");return r
    def record_builder_invocation_consumption_issuance(self,r):
        ...
        self._req_origin(r,"builder invocation consumption issuance")
        with self._lock,self._connect() as c:
    ...
    def resolve_builder_invocation_consumption_issuance(self,i):
        ...
        self._req_origin(r,"builder invocation consumption issuance");return r
    def record_builder_start_admission_issuance(self,r):
        ...
        self._req_origin(r,"builder start admission issuance")
        with self._lock,self._connect() as c:
    ...
    def resolve_builder_start_admission_issuance(self,i):
        ...
        self._req_origin(r,"builder start admission issuance");return r
    def record_builder_process_launch_intent(self,r):
        self._req_origin(r,"builder process launch intent")
        with self._lock,self._connect() as c:
            ...
            except sqlite3.IntegrityError as exc:c.execute("ROLLBACK");raise PersistentAuthorityStateError("launch intent exists or conflicts") from exc
        return r
    def resolve_builder_process_launch_intent(self,i):
        ...
        if len(rows)!=1:raise PersistentAuthorityStateError("launch intent missing or ambiguous")
        r=PersistentBuilderProcessLaunchIntent.from_json(rows[0][0]);self._req_origin(r,"builder process launch intent");return r
    def record_builder_process_held_materialization(self,r):
        self._req_origin(r,"builder process held materialization")
        with self._lock,self._connect() as c:
            try:c.execute("BEGIN IMMEDIATE");c.execute("INSERT INTO builder_process_held_materialization VALUES(?,?,?,?,?,?,?)",(r.launch_id,r.held_identity_digest,r.process_identity_token,r.execution_gate_id,r.execution_gate_digest,r.canonical_json(),r.observed_at));c.execute("COMMIT")
            except sqlite3.IntegrityError as exc:c.execute("ROLLBACK");raise PersistentAuthorityStateError("held materialization exists or conflicts") from exc
        return r
    def resolve_builder_process_held_materialization(self,i):
        ...
        if len(rows)!=1:raise PersistentAuthorityStateError("held materialization missing or ambiguous")
        r=PersistentBuilderProcessHeldMaterialization.from_json(rows[0][0]);self._req_origin(r,"builder process held materialization");return r
    def record_builder_process_launch_receipt(self,r):
        self._req_origin(r,"builder process launch receipt")
        with self._lock,self._connect() as c:
            try:c.execute("BEGIN IMMEDIATE");c.execute("INSERT INTO builder_process_launch_receipt VALUES(?,?,?,?,?,?)",(r.launch_receipt_id,r.launch_receipt_digest,r.launch_replay_digest,r.execution_gate_id,r.canonical_json(),r.launch_observed_at));c.execute("COMMIT")
            except sqlite3.IntegrityError as exc:c.execute("ROLLBACK");raise PersistentAuthorityStateError("launch receipt exists or conflicts") from exc
        return r
    def resolve_builder_process_launch_receipt(self,i):
        ...
        if len(rows)!=1:raise PersistentAuthorityStateError("launch receipt missing or ambiguous")
        r=PersistentBuilderProcessLaunchReceipt.from_json(rows[0][0]);self._req_origin(r,"builder process launch receipt");return r
    def finalize_binding(self,context,*,expected_epoch,expected_state_version,grant_id,expected_root_grant_id,expected_root_grant_digest,live_admission_digest,runtime_evidence_digest,binding_nonce,finalized_at):
        context=self._context(context);key=hashlib.sha256((f"{self.FINALIZATION_DOMAIN}\x00{live_admission_digest}\x00{runtime_evidence_digest}\x00{binding_nonce}").encode()).hexdigest()
        with self._lock,self._connect() as c:
            try:
                ...
                if state is None or int(state[0])!=expected_epoch or int(state[2])!=expected_state_version or grant_id in set(json.loads(state[1])):c.execute("ROLLBACK");raise PersistentAuthorityStateError("authority state changed before binding finalization")
                root=c.execute("SELECT root_grant_id,root_grant_digest FROM authority_root_anchor WHERE trust_domain=? AND tenant_id=? AND organization_id=? AND mission_id=? AND epoch=?",(*context,expected_epoch)).fetchone()

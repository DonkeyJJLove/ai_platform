...
def _sha256(value: object, *, name: str) -> str:
    ...
    if value.lower() != value: raise PersistentAuthorityStateError(f"{name} is invalid")
    return value
...
def _scope(value: object, *, name: str) -> tuple[str, ...]:
    if type(value) is not tuple or not value or len(set(value)) != len(value):
        raise PersistentAuthorityStateError(f"{name} is invalid")
    for item in value: _text(item, name=name, limit=2048)
    return value
...
class PersistentEpochSnapshot:
    ...
    def context(self) -> tuple[str, str, str, str]: return (self.trust_domain, self.tenant_id, self.organization_id, self.mission_id)
...
class PersistentAuthorityStoreOrigin:
    ...
    def validate(self):
        ...
        if self.origin_id != f"aso:{self.origin_digest}": raise PersistentAuthorityStateError("authority store origin id binding mismatch")
        if not Path(self.repository_root).is_absolute() or not Path(self.canonical_database_path).is_absolute(): raise PersistentAuthorityStateError("authority store origin path is invalid")
        return self
    ...
    def from_json(cls,value):
        ...
        if not isinstance(payload,dict) or set(payload)!=set(cls.__dataclass_fields__): raise PersistentAuthorityStateError("authority store origin record is noncanonical")
...
class PersistentBuilderEntryIssuanceRecord:
    ...
    def validate(self):
        ...
        if isinstance(self.authority_epoch,bool) or not isinstance(self.authority_epoch,int) or self.authority_epoch<0: raise PersistentAuthorityStateError("authority_epoch is invalid")
        if isinstance(self.authority_state_version,bool) or not isinstance(self.authority_state_version,int) or self.authority_state_version<1: raise PersistentAuthorityStateError("authority_state_version is invalid")
        return self
...
class PersistentBuilderProcessLaunchIntent:
    launch_request_id:str; launch_request_digest:str; launch_replay_digest:str; source_builder_start_admission_id:str; source_builder_start_admission_digest:str; source_builder_start_admission_replay_digest:str; source_builder_start_issuance_record_id:str; source_builder_start_issuance_record_digest:str; repository:str; baseline_master_sha:str; baseline_master_tree_sha:str; authority_epoch:int; authority_state_version:int; root_grant_id:str; root_grant_digest:str; expected_current_authority_digest:str; builder_subject_id:str; builder_instance_id:str; builder_identity_digest:str; builder_implementation_digest:str; builder_attestation_digest:str; expected_builder_subject_digest:str; process_profile_id:str; process_profile_digest:str; launch_policy_digest:str; runtime_provider_id:str; runtime_provider_identity_digest:str; runtime_provider_implementation_digest:str; runtime_provider_attestation_digest:str; authority_store_origin_id:str; authority_store_origin_digest:str; prepared_at:str
    def validate(self):
        for n in ("launch_request_id","source_builder_start_admission_id","source_builder_start_issuance_record_id","repository","root_grant_id","builder_subject_id","builder_instance_id","process_profile_id","runtime_provider_id","authority_store_origin_id","prepared_at"): _text(getattr(self,n),name=n,limit=2048)
    ...
    def from_request(cls,request,*,authority_store_origin,prepared_at):
        ...
        return cls(
            launch_request_id=request.launch_request_id,launch_request_digest=request.launch_request_digest,launch_replay_digest=request.launch_replay_digest,
            source_builder_start_admission_id=request.source_builder_start_admission_id,source_builder_start_admission_digest=request.source_builder_start_admission_digest,
            source_builder_start_admission_replay_digest=request.source_builder_start_admission_replay_digest,source_builder_start_issuance_record_id=request.source_builder_start_issuance_record_id,
            source_builder_start_issuance_record_digest=request.source_builder_start_issuance_record_digest,repository=request.repository,baseline_master_sha=request.baseline_master_sha,
            baseline_master_tree_sha=request.baseline_master_tree_sha,authority_epoch=request.authority_epoch,authority_state_version=request.authority_state_version,
            root_grant_id=request.root_grant_id,root_grant_digest=request.root_grant_digest,expected_current_authority_digest=request.expected_current_authority_digest,
            builder_subject_id=request.builder_subject_id,builder_instance_id=request.builder_instance_id,builder_identity_digest=request.builder_identity_digest,
            builder_implementation_digest=request.builder_implementation_digest,builder_attestation_digest=request.builder_attestation_digest,
            expected_builder_subject_digest=request.expected_builder_subject_digest,process_profile_id=request.process_profile_id,process_profile_digest=request.process_profile_digest,
            launch_policy_digest=request.launch_policy_digest,runtime_provider_id=request.runtime_provider_id,runtime_provider_identity_digest=request.runtime_provider_identity_digest,
            runtime_provider_implementation_digest=request.runtime_provider_implementation_digest,runtime_provider_attestation_digest=request.runtime_provider_attestation_digest,
            authority_store_origin_id=authority_store_origin.origin_id,authority_store_origin_digest=authority_store_origin.origin_digest,prepared_at=prepared_at,
        ).validate()


@dataclass(frozen=True)
class PersistentBuilderProcessLaunchReceipt:
    launch_receipt_id:str; launch_receipt_digest:str; launch_request_id:str; launch_request_digest:str; launch_replay_digest:str; source_builder_start_admission_id:str; source_builder_start_admission_digest:str; repository:str; baseline_master_sha:str; baseline_master_tree_sha:str; authority_digest_at_launch:str; builder_subject_digest_at_launch:str; process_profile_id:str; process_profile_digest:str; launch_policy_digest:str; runtime_provider_id:str; runtime_provider_identity_digest:str; runtime_provider_implementation_digest:str; runtime_provider_attestation_digest:str; launch_id:str; execution_environment_id:str; process_handle_reference:str; process_identity_token:str; process_identity_digest:str; launch_started_at:str; launch_observed_at:str; effect_class:str; effect_state:str; authority_store_origin_id:str; authority_store_origin_digest:str
    def validate(self):
        for n in ("launch_receipt_id","launch_request_id","source_builder_start_admission_id","repository","process_profile_id","runtime_provider_id","launch_id","execution_environment_id","process_handle_reference","process_identity_token","launch_started_at","launch_observed_at","effect_class","effect_state","authority_store_origin_id"): _text(getattr(self,n),name=n,limit=2048)
    ...
    def from_receipt(cls,receipt,*,authority_store_origin):
        ...
        return cls(
            launch_receipt_id=receipt.launch_receipt_id,launch_receipt_digest=receipt.launch_receipt_digest,launch_request_id=receipt.launch_request_id,
            launch_request_digest=receipt.launch_request_digest,launch_replay_digest=receipt.launch_replay_digest,source_builder_start_admission_id=receipt.source_builder_start_admission_id,
            source_builder_start_admission_digest=receipt.source_builder_start_admission_digest,repository=receipt.repository,baseline_master_sha=receipt.baseline_master_sha,
            baseline_master_tree_sha=receipt.baseline_master_tree_sha,authority_digest_at_launch=receipt.authority_digest_at_launch,builder_subject_digest_at_launch=receipt.builder_subject_digest_at_launch,
            process_profile_id=receipt.process_profile_id,process_profile_digest=receipt.process_profile_digest,launch_policy_digest=receipt.launch_policy_digest,
            runtime_provider_id=receipt.runtime_provider_id,runtime_provider_identity_digest=receipt.runtime_provider_identity_digest,runtime_provider_implementation_digest=receipt.runtime_provider_implementation_digest,
            runtime_provider_attestation_digest=receipt.runtime_provider_attestation_digest,launch_id=receipt.launch_id,execution_environment_id=receipt.execution_environment_id,
            process_handle_reference=receipt.process_handle_reference,process_identity_token=receipt.process_identity_token,process_identity_digest=receipt.process_identity_digest,
            launch_started_at=receipt.launch_started_at,launch_observed_at=receipt.launch_observed_at,effect_class=receipt.effect_class,effect_state=receipt.effect_state,
            authority_store_origin_id=authority_store_origin.origin_id,authority_store_origin_digest=authority_store_origin.origin_digest,
        ).validate()
...
class PersistentBindingFinalization:
    ...
    def validate(self):
        ...
        if isinstance(self.epoch,bool) or not isinstance(self.epoch,int) or self.epoch<0: raise PersistentAuthorityStateError("epoch is invalid")
        if isinstance(self.authority_state_version,bool) or not isinstance(self.authority_state_version,int) or self.authority_state_version<1: raise PersistentAuthorityStateError("authority_state_version is invalid")
        return self
    def canonical_payload(self): self.validate(); return json.dumps(asdict(self),sort_keys=True,separators=(",",":")).encode("utf-8")
...
class SQLiteAuthorityStateStore:
    ...
    REQUIRED_TABLES=frozenset({"authority_epoch_state","authority_root_anchor","replay_state","authority_store_origin","builder_entry_issuance","builder_invocation_issuance","builder_invocation_consumption_issuance","builder_start_admission_issuance","builder_process_launch_intent","builder_process_launch_receipt"})
    ...
    def _context(context):
        if type(context) is not tuple or len(context)!=4 or any(not isinstance(x,str) or not x for x in context): raise PersistentAuthorityStateError("authority context is invalid")
        return context
    @staticmethod
    ...
    def record_builder_entry_issuance(self,record):
        ...
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_entry_issuance VALUES(?,?,?,?,?)",(record.builder_entry_permit_id,record.builder_entry_permit_digest,record.builder_entry_replay_digest,record.canonical_json(),record.issued_at)); c.execute("COMMIT")
    ...
    def resolve_builder_entry_issuance(self,builder_entry_permit_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_entry_issuance WHERE builder_entry_permit_id=?",(builder_entry_permit_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder entry issuance is missing or ambiguous")
        record=PersistentBuilderEntryIssuanceRecord.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder entry issuance"); return record
    def record_builder_invocation_issuance(self,record):
        ...
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_invocation_issuance VALUES(?,?,?,?,?)",(record.builder_invocation_permit_id,record.builder_invocation_permit_digest,record.builder_invocation_replay_digest,record.canonical_json(),record.issued_at)); c.execute("COMMIT")
    ...
    def resolve_builder_invocation_issuance(self,builder_invocation_permit_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_invocation_issuance WHERE builder_invocation_permit_id=?",(builder_invocation_permit_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder invocation issuance is missing or ambiguous")
        record=PersistentBuilderInvocationIssuanceRecord.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder invocation issuance"); return record
    def record_builder_invocation_consumption_issuance(self,record):
        ...
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_invocation_consumption_issuance VALUES(?,?,?,?,?)",(record.invocation_consumption_permit_id,record.invocation_consumption_permit_digest,record.invocation_consumption_replay_digest,record.canonical_json(),record.issued_at)); c.execute("COMMIT")
    ...
    def resolve_builder_invocation_consumption_issuance(self,invocation_consumption_permit_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_invocation_consumption_issuance WHERE invocation_consumption_permit_id=?",(invocation_consumption_permit_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder invocation consumption issuance is missing or ambiguous")
        record=PersistentBuilderInvocationConsumptionIssuanceRecord.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder invocation consumption issuance"); return record
    def record_builder_start_admission_issuance(self,record):
        ...
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_start_admission_issuance VALUES(?,?,?,?,?)",(record.builder_start_admission_id,record.builder_start_admission_digest,record.builder_start_admission_replay_digest,record.canonical_json(),record.issued_at)); c.execute("COMMIT")
    ...
    def resolve_builder_start_admission_issuance(self,builder_start_admission_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_start_admission_issuance WHERE builder_start_admission_id=?",(builder_start_admission_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder start admission issuance is missing or ambiguous")
        record=PersistentBuilderStartAdmissionIssuanceRecord.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder start admission issuance"); return record
    def record_builder_process_launch_intent(self,record):
        if type(record) is not PersistentBuilderProcessLaunchIntent: raise PersistentAuthorityStateError("exact builder process launch intent required")
        self._require_record_origin(record,error_label="builder process launch intent")
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_process_launch_intent VALUES(?,?,?,?,?)",(record.launch_request_id,record.launch_request_digest,record.launch_replay_digest,record.canonical_json(),record.prepared_at)); c.execute("COMMIT")
    ...
    def resolve_builder_process_launch_intent(self,launch_request_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_process_launch_intent WHERE launch_request_id=?",(launch_request_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder process launch intent is missing or ambiguous")
        record=PersistentBuilderProcessLaunchIntent.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder process launch intent"); return record
    def record_builder_process_launch_receipt(self,record):
        if type(record) is not PersistentBuilderProcessLaunchReceipt: raise PersistentAuthorityStateError("exact builder process launch receipt required")
        self._require_record_origin(record,error_label="builder process launch receipt")
        with self._lock,self._connect() as c:
            try: c.execute("BEGIN IMMEDIATE"); c.execute("INSERT INTO builder_process_launch_receipt VALUES(?,?,?,?,?)",(record.launch_receipt_id,record.launch_receipt_digest,record.launch_replay_digest,record.canonical_json(),record.launch_observed_at)); c.execute("COMMIT")
    ...
    def resolve_builder_process_launch_receipt(self,launch_receipt_id):
        ...
        with self._connect() as c: rows=c.execute("SELECT record_json FROM builder_process_launch_receipt WHERE launch_receipt_id=?",(launch_receipt_id,)).fetchall()
        if len(rows)!=1: raise PersistentAuthorityStateError("builder process launch receipt is missing or ambiguous")
        record=PersistentBuilderProcessLaunchReceipt.from_json(rows[0][0]); self._require_resolved_origin(record,error_label="builder process launch receipt"); return record

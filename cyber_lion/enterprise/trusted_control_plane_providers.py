...
R22 executable process capabilities are resolved by a pinned source, never accepted from
an individual launch caller.
"""
...
class SQLiteTrustedControlPlaneStore(TrustedControlPlaneStore):
    BUILDER_LOOKUP_FIELDS = (
        "repository", "builder_subject_id", "builder_instance_id",
        "candidate_scope_digest", "resource_scope_digest", "capability_class",
    )
    ...
    def put_pr_bootstrap(self, record):
        ...
        with self._lock,self._connect() as c:
            c.execute("BEGIN IMMEDIATE"); c.execute("INSERT OR IGNORE INTO pr_bootstrap VALUES(?,?,?,?,?,?)",tuple(lookup[n] for n in fields)+(raw,)); c.execute("COMMIT")
    ...
    def put_authority_record(self, record):
        ...
        with self._lock,self._connect() as c:
            c.execute("BEGIN IMMEDIATE"); c.execute("INSERT OR IGNORE INTO authority_lineage VALUES(?,?,?,?,?,?,?)",tuple(lookup[n] for n in fields)+(raw,)); c.execute("COMMIT")
    ...
    def put_builder_subject_record(self, record):
        ...
        with self._lock,self._connect() as c:
            c.execute("BEGIN IMMEDIATE"); c.execute("INSERT OR IGNORE INTO builder_subject VALUES(?,?,?,?,?,?,?)",tuple(lookup[n] for n in fields)+(raw,)); c.execute("COMMIT")
    ...
    def put_builder_process_runtime_provider_record(self, record):
        ...
        raw=_canonical_json(record)
        with self._lock,self._connect() as c:
            c.execute("BEGIN IMMEDIATE"); c.execute("INSERT OR IGNORE INTO builder_process_runtime_provider VALUES(?,?,?,?,?)",tuple(lookup[n] for n in fields)+(raw,)); c.execute("COMMIT")

    def lookup_pr_bootstrap_exact(self,*,repository,pr_number,base_sha,head_sha,merge_method):
        with self._lock,self._connect() as c:
            rows=c.execute("SELECT record_json FROM pr_bootstrap WHERE repository=? AND pr_number=? AND base_sha=? AND head_sha=? AND merge_method=? ORDER BY record_json",(repository,pr_number,base_sha,head_sha,merge_method)).fetchall()
    ...
    def lookup_authority_exact(self,*,repository,pr_number,base_sha,head_sha,mission_id,grant_id):
        with self._lock,self._connect() as c:
            rows=c.execute("SELECT record_json FROM authority_lineage WHERE repository=? AND pr_number=? AND base_sha=? AND head_sha=? AND mission_id=? AND grant_id=? ORDER BY record_json",(repository,pr_number,base_sha,head_sha,mission_id,grant_id)).fetchall()
        return tuple(_decode_record(x[0]) for x in rows)
    ...
    def lookup_builder_process_runtime_provider_exact(self,*,provider_id,process_profile_digest,launch_policy_digest,capability_class):
        with self._lock,self._connect() as c:
            rows=c.execute("SELECT record_json FROM builder_process_runtime_provider WHERE provider_id=? AND process_profile_digest=? AND launch_policy_digest=? AND capability_class=? ORDER BY record_json",(provider_id,process_profile_digest,launch_policy_digest,capability_class)).fetchall()
        return tuple(_decode_record(x[0]) for x in rows)
...
class PinnedBuilderProcessRuntimeProviderSource:
    """Resolve both the sealed descriptor and the executable runtime capability.

    The launch caller never supplies an executable provider.  A trusted composition root
    constructs this source with a runtime resolver; resolution is pinned by the exact
    provider record and runtime_instance_identity carried by that sealed record.
    """
    __slots__=("_store","_runtime_resolver")

    def __init__(self, store: SQLiteTrustedControlPlaneStore, *, runtime_resolver: Callable[[str], object]):
        if type(store) is not SQLiteTrustedControlPlaneStore or store.ready() is not True:
            raise TrustedControlPlaneProviderError("trusted runtime provider store unavailable")
    ...
    def resolve_exact(self, *, provider_id: str, process_profile_digest: str, launch_policy_digest: str):
        ...
        record=rows[0]
        if record.get("record_kind")!="builder-process-runtime-provider":
            raise TrustedControlPlaneProviderError("runtime provider record kind invalid")
        payload=record.get("provider")
        if not isinstance(payload,Mapping):
            raise TrustedControlPlaneProviderError("runtime provider payload invalid")
        try:
        ...
        if not descriptor.descriptor_digest or descriptor.descriptor_digest!=descriptor.compute_digest():
            raise TrustedControlPlaneProviderError("runtime provider descriptor must be sealed")
        ...
        if not isinstance(record.get("lookup_key"),Mapping) or dict(record["lookup_key"])!=expected:
            raise TrustedControlPlaneProviderError("runtime provider lookup binding mismatch")
        if (descriptor.provider_id,descriptor.supported_process_profile_digest,descriptor.supported_launch_policy_digest,descriptor.capability_class)!=(provider_id,process_profile_digest,launch_policy_digest,PROVIDER_CAPABILITY_CLASS):
    ...
    def resolve_bound_runtime(self, *, provider_id: str, process_profile_digest: str, launch_policy_digest: str):
        ...
        try:
            runtime=self._runtime_resolver(descriptor.runtime_instance_identity)
        except Exception as exc:
        ...
        for method in ("prepare_launch","observe_held","commit_start","observe_launch","freeze_or_kill"):

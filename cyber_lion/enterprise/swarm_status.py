...
""")
    ...
    def _verify_external_file(self):
        try:external=json.loads(self.status_path.read_text(encoding="utf-8"))
        except Exception as exc:raise SwarmStatusStateError("external status file unreadable") from exc
        internal=self._projection()
        for key in ("revision","status_digest","revision_digest"):
            if external.get(key)!=internal.get(key):raise SwarmStatusStateError("external status modification detected")
    ...
    def apply_report(self,report:StatusReport,*,lease:GovernorLease):
        report.validate();self.governor_store.assert_current(lease)
    ...
    def replace_context(self,context:dict,*,operation_id:str,expected_revision:int,expected_status_digest:str,lease:GovernorLease,evidence_refs:tuple[str,...]):
        self.governor_store.assert_current(lease)

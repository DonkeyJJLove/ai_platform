...
from .authority_source import (
    AuthorityLookupKey, AuthorityLineageRecord, AuthoritySource,
    RepositoryRefAuthorityLookupKey, RepositoryRefAuthorityLineageRecord,
)
...
@dataclass(frozen=True)
class LiveAdmittedRepositoryRefAuthority:
    repository: str
    branch: str
    expected_head: str
    ...
    def validate(self) -> "LiveAdmittedRepositoryRefAuthority":
        for name in (
            "repository", "branch", "expected_head", "protected_master_sha", "mission_id",
            "grant_id", "provenance_id", "authority_ceiling", "root_grant_id",
            "leaf_key_id", "leaf_algorithm", "admitted_at",
        ):
            _text(getattr(self, name), name=name)
        if not isinstance(self.epoch, int) or isinstance(self.epoch, bool) or self.epoch < 0:
            raise LiveAuthorityAdmissionError("epoch is invalid")
        ...
        for item in self.authenticated_grant_digests:
            _sha256(item, name="authenticated_grant_digest")
        LiveAdmittedAuthority._utc(self.admitted_at)
        RepositoryRefAuthorityLookupKey(
            self.repository, self.branch, self.expected_head, self.protected_master_sha,
            self.mission_id, self.grant_id,
        ).validate()
        return self
        ...
        return json.dumps(
            {
                ...
                "lineage_digest": self.lineage_digest,
                "mission_id": self.mission_id,
                "protected_master_sha": self.protected_master_sha,
                "provenance_id": self.provenance_id,
                "replay_digest": self.replay_digest,
                "repository": self.repository,
                "root_grant_digest": self.root_grant_digest,
...
class LiveAuthorityAdmission:
    ...
    def _snapshot_repository_ref(
        self, key: RepositoryRefAuthorityLookupKey
    ) -> tuple[RepositoryRefAuthorityLineageRecord, PersistentEpochSnapshot, PersistentRootAnchor]:
        try:
            record = self._source.resolve_repository_ref_exact(key)
            state = self._epoch_provider.current(self._context_key())
            root = self._root_provider.resolve(self._context_key(), state.epoch)
        except Exception as exc:
            raise LiveAuthorityAdmissionError("canonical repository-ref authority state unavailable") from exc
        record.validate()
        return record, state, root
    ...
    def admit_repository_ref(
        self,
        *,
        key: RepositoryRefAuthorityLookupKey,
        now: datetime,
        replay_nonce: str,
    ) -> LiveAdmittedRepositoryRefAuthority:
        if type(key) is not RepositoryRefAuthorityLookupKey:
            raise LiveAuthorityAdmissionError("repository-ref authority key is invalid")
        key.validate()
        if now.tzinfo is None:
            raise LiveAuthorityAdmissionError("trusted now must be timezone-aware")
        ...
        if not isinstance(replay_nonce, str) or not replay_nonce.strip():
            raise LiveAuthorityAdmissionError("replay_nonce is required")
        before = self._snapshot_repository_ref(key)
        record, state, root = before
        authenticated = self._authenticate_record(record, state, root, now=now)
        after = self._snapshot_repository_ref(key)
        if not self._same_snapshot(before, after):
            raise LiveAuthorityAdmissionError("authority state changed during authentication")
        leaf = record.lineage[-1]
        replay_material = (
            f"{record.lineage_digest}\x00{leaf.grant_id}\x00{key.repository}\x00{key.branch}\x00"
            f"{key.expected_head}\x00{key.protected_master_sha}\x00{key.mission_id}\x00"
            f"{state.epoch}\x00{state.version}\x00{replay_nonce}"
        ).encode("utf-8")
        replay_digest = hashlib.sha256(replay_material).hexdigest()
        current = now.astimezone(timezone.utc)
        try:
        ...
        if accepted is not True:
            raise LiveAuthorityAdmissionError("authority admission replay rejected")
        return LiveAdmittedRepositoryRefAuthority(
            repository=key.repository, branch=key.branch, expected_head=key.expected_head,
            protected_master_sha=key.protected_master_sha, mission_id=key.mission_id,
            grant_id=leaf.grant_id, lineage_digest=record.lineage_digest,
            provenance_id=record.provenance_id, epoch=state.epoch, epoch_state_version=state.version,
            authority_ceiling=leaf.authority_ceiling, root_grant_id=root.root_grant_id,
            root_grant_digest=root.root_grant_digest,
            authenticated_grant_digests=tuple(item.grant_digest for item in authenticated),
            leaf_key_id=authenticated[-1].key_id, leaf_algorithm=authenticated[-1].algorithm,
            replay_digest=replay_digest, admitted_at=current.isoformat(),
        ).validate()
    ...
    def revalidate_repository_ref(
        self, admitted: LiveAdmittedRepositoryRefAuthority, *, now: datetime
    ) -> LiveAdmittedRepositoryRefAuthority:
        if type(admitted) is not LiveAdmittedRepositoryRefAuthority:
            raise LiveAuthorityAdmissionError("repository-ref live admission receipt has invalid type")
        admitted.validate()
        if now.tzinfo is None:
            raise LiveAuthorityAdmissionError("trusted now must be timezone-aware")
        if admitted.mission_id != self._context.mission_id:
            raise LiveAuthorityAdmissionError("admission receipt mission mismatch")
        key = RepositoryRefAuthorityLookupKey(
            admitted.repository, admitted.branch, admitted.expected_head, admitted.protected_master_sha,
            admitted.mission_id, admitted.grant_id,
        ).validate()
        before = self._snapshot_repository_ref(key)
        record, state, root = before
        authenticated = self._authenticate_record(record, state, root, now=now)
        ...
        expected = (
            ...
            tuple(item.grant_digest for item in authenticated), authenticated[-1].key_id,
            authenticated[-1].algorithm,
        )
        actual = (
            admitted.lineage_digest, admitted.provenance_id, admitted.epoch, admitted.epoch_state_version,
            admitted.authority_ceiling, admitted.root_grant_id, admitted.root_grant_digest,
            admitted.authenticated_grant_digests, admitted.leaf_key_id, admitted.leaf_algorithm,
        )
        if actual != expected:
            raise LiveAuthorityAdmissionError("live admission receipt is stale or forged")
        return admitted

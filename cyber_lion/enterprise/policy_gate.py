...
from .authority_source import AuthorityLookupKey,RepositoryRefAuthorityLookupKey
...
class CanonicalPolicyDecisionPoint:
    ...
    def evaluate(self,*,request_id:str,gate_event_id:str,proposal:ActionProposal,mission:MissionSpec,swarm:SwarmSpec,agents:Mapping[str,AgentSpec],policy:PolicyRevision,authority_key:AuthorityLookupKey|RepositoryRefAuthorityLookupKey,graph_projection:EnterpriseGraphProjection,status:Mapping[str,object],observability_state:str,observed_event_types:tuple[str,...],evidence_refs:tuple[str,...],trusted_now:datetime)->PDPResult:
        ...
        if old is not None:
            ...
            return old[1]
        try:
            if type(authority_key) is AuthorityLookupKey:
            ...
            elif type(authority_key) is RepositoryRefAuthorityLookupKey:
                admitted=self.authority_admission.admit_repository_ref(key=authority_key,now=trusted_now,replay_nonce=f"pdp:{request_id}")
                lineage=self.authority_admission._source.resolve_repository_ref_exact(authority_key)
            else:
                raise PolicyGateError("unsupported authority lookup key type")
        ...
        try:
            if type(authority_key) is AuthorityLookupKey:self.authority_admission.revalidate(admitted,now=trusted_now)
            else:self.authority_admission.revalidate_repository_ref(admitted,now=trusted_now)

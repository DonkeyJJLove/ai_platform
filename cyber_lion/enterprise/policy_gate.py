...
_CONTAINS={
 "none":{"none"},"read":{"none","read"},"local_write":{"none","read","local_write"},
...
def _hex64(value:object,name:str)->str:
    if not isinstance(value,str) or len(value)!=64 or any(c not in "0123456789abcdef" for c in value):raise PolicyGateError(f"{name} must be canonical sha256 hex")
    return value
...
def _graph_digest(graph_projection:object)->str:
    verifier=getattr(graph_projection,"verify_digest",None)
    if callable(verifier):
        try:graph_projection=verifier()
        except Exception as exc:raise PolicyGateError("EnterpriseGraph projection digest invalid") from exc
    return _hex64(getattr(graph_projection,"projection_digest",None),"enterprise_graph_digest")
...
def _pre_request_digest(*,request_id,gate_event_id,proposal,policy,authority_key,graph_digest,status_digest,observability_state,observed_event_types,evidence_refs):
    payload={"request_id":request_id,"gate_event_id":gate_event_id,"proposal":asdict(proposal),"policy":asdict(policy),
        "authority_key":authority_key.binding(),"graph_digest":graph_digest,"status_digest":status_digest,
        "observability_state":observability_state,"observed_event_types":list(observed_event_types),"evidence_refs":list(evidence_refs)}
    return sha256(json.dumps(payload,sort_keys=True,separators=(",",":"),default=list).encode()).hexdigest()

@dataclass(frozen=True)
class PDPResult:
    requested:GateRequested;applied:GateApplied;receipt:PDPDecisionReceipt
...
class CanonicalPolicyDecisionPoint:
    ...
    def __init__(self,*,authority_admission:LiveAuthorityAdmission,control_plane:ExecutionControlPlane|None=None):
        ...
        self.authority_admission=authority_admission;self.control_plane=control_plane or ExecutionControlPlane()
    ...
    def evaluate(self,*,request_id:str,gate_event_id:str,proposal:ActionProposal,mission:MissionSpec,
                 swarm:SwarmSpec,agents:Mapping[str,AgentSpec],policy:PolicyRevision,authority_key:AuthorityLookupKey,
                 graph_projection:object,status:Mapping[str,object],observability_state:str,observed_event_types:tuple[str,...],
                 evidence_refs:tuple[str,...],trusted_now:datetime)->PDPResult:
        ...
        pre=_pre_request_digest(request_id=request_id,gate_event_id=gate_event_id,proposal=proposal,policy=policy,
            authority_key=authority_key,graph_digest=gd,status_digest=sd,observability_state=observability_state,
            observed_event_types=observed_event_types,evidence_refs=evidence_refs)
        old=self._replay.get(request_id)
        if old is not None:
            if old[0]!=pre:raise PolicyGateError("replayed request_id payload substitution denied")
            return old[1]
        try:
            admitted=self.authority_admission.admit(repository=authority_key.repository,pr_number=authority_key.pr_number,
                base_sha=authority_key.base_sha,head_sha=authority_key.head_sha,mission_id=authority_key.mission_id,
                grant_id=authority_key.grant_id,now=trusted_now,replay_nonce=f"pdp:{request_id}")
            # The admission owns this canonical source; resolve only to bind signed grant semantics
            # (capability/policy digest) to the freshly authenticated/current admission result.
        ...
        if lineage.lineage_digest!=admitted.lineage_digest or leaf.digest()!=admitted.leaf_grant_digest:
            raise PolicyGateError("live admission/lineage binding mismatch")
        ...
        request=GateRequested(request_id=request_id,proposal_id=proposal.proposal_id,policy_binding=policy.binding,
            authority_lineage_digest=admitted.lineage_digest,enterprise_graph_digest=gd,status_digest=sd,
            observability_state=observability_state,lane=policy.lane,requested_authority=proposal.requested_authority,evidence_refs=evidence_refs).sealed()
        ...
        applied=GateApplied(gate_event_id=gate_event_id,request_id=request_id,proposal_id=proposal.proposal_id,decision=decision,
            effective_authority=effective,policy_binding=policy.binding,authority_lineage_digest=admitted.lineage_digest,
            enterprise_graph_digest=gd,status_digest=sd,observability_state=observability_state,lane=policy.lane,rationale=rationale).sealed()
        replay_key=sha256((request.request_digest+applied.decision_digest+admitted.digest()).encode("ascii")).hexdigest()
        receipt=PDPDecisionReceipt(receipt_id=f"pdp-receipt:{request_id}",request_id=request_id,gate_event_id=gate_event_id,
            request_digest=request.request_digest,decision_digest=applied.decision_digest,replay_key=replay_key).validate()
        result=PDPResult(request,applied,receipt);self._replay[request_id]=(pre,result);return result

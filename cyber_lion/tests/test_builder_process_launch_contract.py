...
def descriptor():
    return BuilderProcessRuntimeProviderDescriptor(
        ...
        capability_class="BUILDER_PROCESS_START_ONLY", supported_process_profile_digest="d"*64,
        supported_launch_policy_digest="e"*64, isolation_class="HELD_PROCESS",
        process_identity_scheme="stable-handle", observation_scheme="provider-observed",
        recovery_scheme="freeze-or-kill",
    ).sealed()
...
class BuilderProcessLaunchContractTests(unittest.TestCase):
    def test_provider_descriptor_is_sealed_and_non_authoritative(self):
    ...
    def test_request_is_deterministic_and_non_effectful(self):
    ...
    def test_replay_changes_when_provider_attestation_changes(self):
        a=request(); changed=replace(a,runtime_provider_attestation_digest="f"*64,launch_replay_digest="",launch_request_digest="")
        replay=compute_launch_replay_digest(**changed.replay_kwargs()); self.assertNotEqual(replay,a.launch_replay_digest)
    ...
    def test_pid_alone_is_denied(self):
        with self.assertRaises(BuilderProcessLaunchContractError):
            BuilderProcessIdentity(launch_id="launch:r22",builder_subject_id="builder:r22",builder_instance_id="instance:r22",
                ...
                runtime_provider_id="provider:r22:test",runtime_provider_identity_digest=D,execution_environment_id="env:r22",
                process_handle_reference="1234",process_identity_token="token:r22",started_at="2026-08-25T14:00:00Z",state=HELD_STATE).validate()
    ...
    def test_receipt_is_first_effect_only(self):
        r=request(); ident=BuilderProcessIdentity(launch_id="launch:r22",builder_subject_id="builder:r22",builder_instance_id="instance:r22",
            ...
            execution_environment_id="env:r22",process_handle_reference="pidfd:r22:1",process_identity_token="token:r22",
            started_at="2026-08-25T14:00:00Z",state=STARTED_STATE).sealed()
        receipt=BuilderProcessLaunchReceipt(launch_receipt_id=f"bplx:{r.launch_replay_digest}",launch_request_id=r.launch_request_id,
            ...
            launch_id=ident.launch_id,execution_environment_id=ident.execution_environment_id,process_handle_reference=ident.process_handle_reference,
            process_identity_token=ident.process_identity_token,process_identity_digest=ident.identity_digest,

...
def fixture(authority="external_write",risk="GREEN",requested="read",verifier=None):
    key=AuthorityLookupKey("DonkeyJJLove/ai_platform",123,"a"*40,"b"*40,"m","g").validate();src=Source(grant_for(key,authority));pdp=CanonicalPolicyDecisionPoint(authority_source=src)
    ...
    prop=ActionProposal("p","m","s","a","cap",requested,"act","target",evidence_refs=("ev",),required_observability=("trace",),verifier_agent_id=verifier).validate();policy=PolicyRevision("policy","1","sha256:"+Z,risk).validate()
...
def evaluate(parts,**overrides):
    ...
    args=dict(request_id="r",gate_event_id="g",proposal=prop,mission=mission,swarm=swarm,agents=agents,policy=policy,authority_key=key,graph_projection=Graph(),status={"epistemic_state":"CURRENT","status_digest":"2"*64},observability_state="HEALTHY",observed_event_types=("trace",),evidence_refs=("graph:1","status:1","authority:1"));args.update(overrides);return pdp.evaluate(**args)

class CanonicalPDPTests(unittest.TestCase):
    def test_green_allow_exact_evidence_and_authority(self):
        out=evaluate(fixture());self.assertEqual(out.applied.decision,"ALLOW");self.assertEqual(out.receipt.request_digest,out.requested.request_digest)
    def test_semantic_partial_order_keeps_financial_and_deploy_incomparable(self):
        self.assertFalse(authority_contains("financial","deploy"));self.assertFalse(authority_contains("deploy","financial"))
        with self.assertRaises(PolicyGateError): evaluate(fixture(authority="financial",requested="deploy"))
    def test_stale_status_fails_closed(self):
        with self.assertRaises(PolicyGateError): evaluate(fixture(),status={"epistemic_state":"STALE","status_digest":"2"*64})
    def test_degraded_observability_monotonically_denies_write(self):
        out=evaluate(fixture(requested="external_write"),observability_state="DEGRADED");self.assertEqual(out.applied.decision,"DENY");self.assertEqual(out.applied.effective_authority,"none")
    def test_replay_substitution_is_denied(self):

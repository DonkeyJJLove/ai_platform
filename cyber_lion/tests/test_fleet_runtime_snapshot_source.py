...
def create_status_db(path: Path, *, active: bool = False, include_repository_decision: bool = True) -> None:
    ...
    try:
        ...
        CREATE TABLE fleet_meta(
            singleton INTEGER PRIMARY KEY, registry_instance_id TEXT, revision INTEGER,
            event_head TEXT, receipt_head TEXT
        );
        CREATE TABLE fleet_identity(
            mission_id TEXT PRIMARY KEY, executor_id TEXT, repository TEXT,
            baseline_sha TEXT, baseline_tree_sha TEXT, branch TEXT
        );
        ...
        CREATE TABLE fleet_heartbeat(
            mission_id TEXT PRIMARY KEY, deadline_seconds INTEGER, observed_at TEXT
        );
        CREATE TABLE fleet_projection(
            mission_id TEXT, kind TEXT, state TEXT, observed_at TEXT,
            PRIMARY KEY(mission_id,kind)
        );
        CREATE TABLE fleet_verification(
            mission_id TEXT PRIMARY KEY, verification_state TEXT
        );
        CREATE TABLE fleet_lease(lease_id TEXT PRIMARY KEY, mission_id TEXT, state TEXT);
        CREATE TABLE fleet_event(
            seq INTEGER PRIMARY KEY, event_type TEXT, mission_id TEXT, payload_json TEXT,
            previous_digest TEXT, event_digest TEXT, observed_at TEXT
        );
        CREATE TABLE fleet_receipt(
            seq INTEGER PRIMARY KEY, receipt_id TEXT, mission_id TEXT, source_ref TEXT,
            previous_digest TEXT, receipt_digest TEXT, observed_at TEXT
        );
        CREATE TABLE fleet_source_decision(
            seq INTEGER PRIMARY KEY, mission_id TEXT, dimension TEXT,
            decision_type TEXT, decision_json TEXT
        );
        """)
        ...
        conn.execute(
            "INSERT INTO fleet_meta VALUES(1,?,?,?,?)",
            ("status-01", 1, ZERO, ZERO if active else rd),
        )
        conn.execute(
            "INSERT INTO fleet_identity VALUES(?,?,?,?,?,?)",
            (MISSION, EXECUTOR, REPO, BASELINE, BASELINE_TREE, BRANCH),
        )
        conn.execute("INSERT INTO fleet_mission VALUES(?,?)", (MISSION, state))
        conn.execute("INSERT INTO fleet_runtime VALUES(?,?)", (MISSION, RUNTIME))
        conn.execute(
            "INSERT INTO fleet_verification VALUES(?,?)",
            (MISSION, "PASS"),
        )
        if active:
            conn.execute(
                "INSERT INTO fleet_heartbeat VALUES(?,?,?)",
                (MISSION, 5, (NOW - timedelta(seconds=60)).isoformat()),
            )
        for kind, value in (
            ...
            conn.execute(
                "INSERT INTO fleet_projection VALUES(?,?,?,?)",
                (MISSION, kind, value, observed),
            )
        if active:
            conn.execute(
                "INSERT INTO fleet_lease VALUES(?,?,?)",
                ("write-lease-1", MISSION, "ACTIVE"),
            )
        else:
            conn.execute(
                "INSERT INTO fleet_receipt VALUES(1,?,?,?,?,?,?)",
                (rid, MISSION, "result:done", ZERO, rd, observed),
            )
        if include_repository_decision:
            conn.execute(
                "INSERT INTO fleet_source_decision VALUES(1,?,?,?,?)",
                (MISSION, "REPOSITORY", "FACT", repository_decision_json()),
            )
        conn.commit()
    finally:
        conn.close()
...
def create_coordination_db(path: Path, *, active: bool = False) -> None:
    ...
    try:
        conn.executescript("""
        CREATE TABLE fleet_coordination_meta(
            singleton INTEGER PRIMARY KEY, coordinator_id TEXT,
            revision INTEGER, event_head TEXT
        );
        ...
        """)
        conn.execute(
            "INSERT INTO fleet_coordination_meta VALUES(1,?,?,?)",
            ("coord-01", 1, ZERO),
        )
        if active:
            dispatch = "d" * 64
            ...
            conn.execute(
                ...
                (MISSION, "RUNNING", 1, dispatch, fence, BRANCH, NOW.isoformat()),
            )
            conn.execute(
                "INSERT INTO fleet_coordination_active_lease VALUES(?,?,?,?,?,?)",
                (REPO, "BRANCH", BRANCH, MISSION, dispatch, 1),
            )
            conn.execute(
                "INSERT INTO fleet_coordination_active_lease VALUES(?,?,?,?,?,?)",
                (REPO, "PATH", "cyber_lion/x.py", MISSION, dispatch, 1),
            )
        else:
            conn.execute(
                "INSERT INTO fleet_coordination_mission VALUES(?,?,?,?,?,?,?)",
                (MISSION, "DONE", 1, "d" * 64, "f" * 64, BRANCH, NOW.isoformat()),
            )
        conn.commit()
    finally:
        conn.close()
...
def create_reconciliation_db(
    path: Path,
    *,
    master: str = MASTER,
    converged: bool = True,
    consumed: int = 0,
    purpose: str = RECEIPT_PURPOSE,
    receipt_preconditions: str | None = None,
) -> None:
    conn = sqlite3.connect(path)
    try:
        conn.executescript("""
        ...
        report_digest = "2" * 64
        pre = "3" * 64
        receipt_pre = receipt_preconditions or pre
        report_id = "report-1"
        inventory_id = "inventory-1"
        issued_at = NOW.isoformat()
        conn.execute(
            "INSERT INTO reconciliation_inventory_head VALUES(?,?,?,?,?,?)",
            (REPO, inventory_id, 1, inv, master, NOW.isoformat()),
        )
        conn.execute(
            "INSERT INTO reconciliation_report VALUES(?,?,?,?,?,?,?,?,?,?)",
            (
                report_digest, report_id, REPO, inventory_id, 1, inv, pre, master,
                "CONVERGED" if converged else "RECONCILIATION_REQUIRED", NOW.isoformat(),
            ),
        )
        canonical = ConvergenceReceipt.build(
            ...
            closure_preconditions_digest=receipt_pre,
            default_head_sha=master,
            issued_at=issued_at,
        )
        # Purpose is part of the canonical digest. If a noncanonical purpose is requested,
        # preserve the canonical digest so the producer must reject the tampered row.
        conn.execute(
            "INSERT INTO convergence_receipt VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
            (
                canonical.receipt_digest, canonical.receipt_id, report_digest, REPO,
                inventory_id, 1, inv, receipt_pre, master, issued_at, purpose, consumed,
            ),
        )
        conn.commit()
    finally:
...
def append_orphan_receipt(path: Path) -> None:
    conn = sqlite3.connect(path)
    try:
        meta = conn.execute("SELECT receipt_head FROM fleet_meta WHERE singleton=1").fetchone()
        previous = str(meta[0])
        ...
        conn.execute(
            "INSERT INTO fleet_receipt VALUES(2,?,?,?,?,?,?)",
            ("orphan-result", "ORPHAN", "result:orphan", previous, digest, observed),
        )
        conn.execute("UPDATE fleet_meta SET receipt_head=? WHERE singleton=1", (digest,))
        conn.commit()
    finally:
        conn.close()
...
class RuntimeSnapshotSourceTests(unittest.TestCase):
    ...
    def test_query_only_connection_rejects_write(self):
        self.create_valid_sources()
        conn = _ro_connect(str(self.status))
        try:
            self.assertEqual(conn.execute("PRAGMA query_only").fetchone()[0], 1)
            with self.assertRaises(sqlite3.OperationalError):
                conn.execute("DELETE FROM fleet_mission")
        finally:
            conn.close()
    ...
    def test_missing_runtime_registry_table_denied(self):
        self.create_valid_sources()
        conn = sqlite3.connect(self.status)
    ...
    def test_missing_verification_registry_table_denied(self):
        self.create_valid_sources()
        conn = sqlite3.connect(self.status)
        conn.execute("DROP TABLE fleet_verification")
        conn.commit()
        conn.close()
        with self.assertRaises(RuntimeSnapshotSourceError):
            observe_runtime_state(self.config(), clock=lambda: NOW)
    ...
    def test_runtime_inventory_mismatch_fails_closed(self):
        self.create_valid_sources()
        conn = sqlite3.connect(self.status)
        conn.execute("DELETE FROM fleet_runtime WHERE mission_id=?", (MISSION,))
        conn.commit()
        conn.close()
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertFalse(observed.inventory_complete)
        self.assertGreater(observed.unknown_missions, 0)

    def test_verification_inventory_mismatch_fails_closed(self):
        self.create_valid_sources()
        conn = sqlite3.connect(self.status)
        conn.execute("DELETE FROM fleet_verification WHERE mission_id=?", (MISSION,))
        conn.commit()
        conn.close()
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertFalse(observed.inventory_complete)
    ...
    def test_orphan_result_is_counted_and_blocks_inventory(self):
        self.create_valid_sources()
        append_orphan_receipt(self.status)
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertEqual(observed.unknown_results, 1)
        self.assertFalse(observed.inventory_complete)

    def test_repository_ownership_evidence_is_required(self):
        create_status_db(self.status, include_repository_decision=False)
        create_coordination_db(self.coordination)
        create_reconciliation_db(self.reconciliation)
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertGreater(observed.unknown_branch_ownership, 0)
        self.assertFalse(observed.inventory_complete)
    ...
    def test_repository_ownership_mismatch_fails_closed(self):
        ...
        conn = sqlite3.connect(self.status)
        conn.execute(
            "UPDATE fleet_source_decision SET decision_json=? WHERE dimension='REPOSITORY'",
            (repository_decision_json(branch="mission/wrong"),),
        )
        conn.commit()
        conn.close()
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
    ...
    def test_malformed_critical_decision_denied(self):
        ...
        conn = sqlite3.connect(self.status)
        conn.execute(
            "INSERT INTO fleet_source_decision VALUES(2,?,?,?,?)",
            (MISSION, "AUTHORITY", "BROKEN", "{}"),
        )
        conn.commit()
        conn.close()
        with self.assertRaises(RuntimeSnapshotSourceError):
    ...
    def test_consumed_reconciliation_receipt_is_replay_denied(self):
        create_status_db(self.status)
        create_coordination_db(self.coordination)
        create_reconciliation_db(self.reconciliation, consumed=1)
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertFalse(observed.inventory_complete)
    ...
    def test_wrong_reconciliation_receipt_purpose_is_denied(self):
        create_status_db(self.status)
        create_coordination_db(self.coordination)
        create_reconciliation_db(self.reconciliation, purpose="FLEET_CLOSURE")
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertFalse(observed.inventory_complete)
        self.assertGreater(observed.reconciliation_disagreements, 0)
    ...
    def test_closure_preconditions_digest_mismatch_is_denied(self):
        create_status_db(self.status)
        create_coordination_db(self.coordination)
        create_reconciliation_db(self.reconciliation, receipt_preconditions="5" * 64)
        observed = observe_runtime_state(self.config(), clock=lambda: NOW)
        self.assertFalse(observed.inventory_complete)
        self.assertGreater(observed.reconciliation_disagreements, 0)

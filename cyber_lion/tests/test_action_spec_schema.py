...
BASELINE_SHA = "cf95092b2fe4be0a254d5e278a339553a99aa34d"
BASELINE_TREE = "22e5b6ba07f190c23cf886fef94ab7508adba7a9"
...
class ActionSpecCanonicalContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_baseline_and_fail_closed_metadata_are_exact(self):
        ...
        self.assertEqual(self.schema["$id"], "cyberlion://schemas/action-spec/v1")
        self.assertEqual(self.schema["title"], "Cyber-Lion ActionSpec v1")
        self.assertEqual(self.schema["properties"]["schema_version"], {"const": "1.0.0"})
        self.assertEqual(self.matrix["canonical_schema"], "cyber_lion/contracts/v1/action_spec.schema.json")
        self.assertEqual(self.matrix["contract"]["status"], "CONTRACT_ONLY")
        self.assertTrue(self.matrix["contract"]["action_proposal_runtime_envelope_preserved"])
        ...
        self.assertEqual(c0["status"], "CONTRACT_ONLY")
        self.assertEqual(c0["supersedes"], "NONE")

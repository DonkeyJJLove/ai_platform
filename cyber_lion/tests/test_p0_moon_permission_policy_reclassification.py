...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"389765e5a3763020f4ef601539aaa739fab1f43cbb0347b395a8711da86389db");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),466)
    ...
    def test_global_carrier_remains_fail_closed(self):
        ...
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":459});self.assertEqual(self.carrier.global_status,"UNKNOWN")

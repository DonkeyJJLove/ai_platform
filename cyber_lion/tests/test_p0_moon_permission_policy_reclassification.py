...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"621c35a00a31af10a1b556f59c3c5da0bd969b6f269cecfae2777132eb990b36");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),273)
    ...
    def test_global_carrier_remains_fail_closed(self):
        ...
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":266});self.assertEqual(self.carrier.global_status,"UNKNOWN")

...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"033d80f54fc54f319c922efeabe02bbf6d41e015ec41ca6ef570bbf1933908ee");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),248)
    ...
    def test_global_carrier_remains_fail_closed(self):
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":241});self.assertEqual(self.carrier.global_status,"UNKNOWN")

...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"b42da5103dc22429e7367e38fd5deb9331abe5d50d7a8f957baa448bff38384a");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),245)
    ...
    def test_global_carrier_remains_fail_closed(self):
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":238});self.assertEqual(self.carrier.global_status,"UNKNOWN")

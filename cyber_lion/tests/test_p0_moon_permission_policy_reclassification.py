...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"c8a9808505610881c48f827e9e7158287588f15c0ba8e4020f5cf0777d7f0ed0");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),412)
    ...
    def test_global_carrier_remains_fail_closed(self):
        ...
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":405});self.assertEqual(self.carrier.global_status,"UNKNOWN")

...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"146c183e157f0123ded29b1e82cd2a4914cbbce32716efbf645300ca1b222123");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),414)
    ...
    def test_global_carrier_remains_fail_closed(self):
        ...
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":407});self.assertEqual(self.carrier.global_status,"UNKNOWN")

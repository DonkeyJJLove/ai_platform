...
class PermissionPolicyReclassificationTests(unittest.TestCase):
    ...
    def test_current_inventory_and_taxonomy_remain_exact(self):
        self.assertEqual(self.inv.scan_digest,"49f1e1aff202ea1935e15e24200d9208c7a86fbfe12763bfe02b44459ff6a3a8");self.assertFalse(self.tax.unresolved_refs);self.assertEqual(len(self.inv.surfaces),239)
    ...
    def test_global_carrier_remains_fail_closed(self):
        counts={s:sum(x.status==s for x in self.carrier.surface_statuses) for s in ("MEDIATED","PARTIAL","UNMEDIATED","UNKNOWN")};self.assertEqual(counts,{"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":232});self.assertEqual(self.carrier.global_status,"UNKNOWN")

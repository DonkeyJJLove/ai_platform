...
class SwarmStatusTests(unittest.TestCase):
    def _stores(self,d):
        c=Clock();g=SwarmGovernorLeaseStore(pathlib.Path(d)/"g.db",clock=c);lease=g.acquire("g",lease_seconds=60);s=SwarmStatusStore(pathlib.Path(d)/"s.db",pathlib.Path(d)/"status.json",system_id="LION",clock=c,governor_store=g,known_drones=("d1",),initial_context=ctx());return c,g,lease,s
    def test_action_lifecycle_moves_terminal_to_history(self):
        with tempfile.TemporaryDirectory() as d:
            c,g,lease,s=self._stores(d);x=s.snapshot();x=s.apply_report(report(x,"1","ACTION_PLANNED",{"action_id":"a","role":"BUILDER"}),lease=lease);x=s.apply_report(report(x,"2","ACTION_ACCEPTED",{"action_id":"a"}),lease=lease);x=s.apply_report(report(x,"3","ACTION_STARTED",{"action_id":"a"}),lease=lease);x=s.apply_report(report(x,"4","ACTION_COMPLETED",{"action_id":"a","result_refs":["pr:1"]}),lease=lease);self.assertEqual(x["current_actions"],[]);self.assertEqual(x["history"][0]["state"],"COMPLETED");validate_status_projection(x)
    def test_stale_cas_and_unknown_drone_fail_closed(self):
        with tempfile.TemporaryDirectory() as d:
            c,g,lease,s=self._stores(d);x=s.snapshot();r=report(x,"1","ACTION_PLANNED",{"action_id":"a"});s.apply_report(r,lease=lease)

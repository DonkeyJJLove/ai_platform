...
class RepositoryMaintenanceWorkflowTests(unittest.TestCase):
    ...
    def test_issue_comment_trigger_is_exact_and_owner_validation_is_runtime_step(self):
        self.assertIn("issue_comment:", self.workflow)
        self.assertIn("github.event.issue.number == 144", self.workflow)
        self.assertIn("github.event.comment.body == 'LION-BRANCH-CLEANUP v1'", self.workflow)
        self.assertIn("Validate repository-owner control actor", self.workflow)

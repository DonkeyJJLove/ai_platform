    ...
    def test_panel_pair_bind_send_and_read_share_exact_operator_correlation(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                status,sent=req('/api/threads/'+tid+'/bus',{'content':'status now'},csrf);self.assertEqual(status,201);self.assertEqual(g.commands[0]['correlation_id'],tid);self.assertEqual(g.commands[0]['target'],'drone:MD025')

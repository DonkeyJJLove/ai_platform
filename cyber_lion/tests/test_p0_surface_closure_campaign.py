...
EXPECTED_CLASSES={
    "persistent_state.write":190,"filesystem.write":12,"filesystem.delete":11,"runtime.tool_execution":8,"filesystem.replace":5,
...
class P0SurfaceClosureCampaignTests(unittest.TestCase):
    ...
    def test_no_surface_is_synthetically_mediated(self):
        _,c=campaign();self.assertEqual(sum(x.closure_status=="PARTIAL" for x in c.work_items),7);self.assertEqual(sum(x.closure_status=="UNKNOWN" for x in c.work_items),233)
    ...
    def test_provider_family_partition_and_concentration(self):
        ...
        self.assertEqual(len(multi),32);self.assertEqual(sum(len(f.surface_digests) for f in multi),227);self.assertEqual(len(single),13)

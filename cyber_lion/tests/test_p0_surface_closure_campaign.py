...
EXPECTED_CLASSES={
    "persistent_state.write":230,"filesystem.write":14,"filesystem.delete":14,"runtime.tool_execution":9,"filesystem.replace":7,
...
class P0SurfaceClosureCampaignTests(unittest.TestCase):
    def test_exact_current_matrix_and_scan_digest(self):
        inv,c=campaign();self.assertEqual(inv.scan_digest,EXPECTED_SCAN_DIGEST);self.assertEqual(len(inv.surfaces),289);self.assertEqual(c.remaining_surface_count,288)
    ...
    def test_no_surface_is_synthetically_mediated(self):
        _,c=campaign();self.assertEqual(sum(x.closure_status=="PARTIAL" for x in c.work_items),7);self.assertEqual(sum(x.closure_status=="UNKNOWN" for x in c.work_items),281)
    ...
    def test_provider_family_partition_and_concentration(self):
        ...
        self.assertEqual(len(multi),41);self.assertEqual(sum(len(f.surface_digests) for f in multi),274);self.assertEqual(len(single),14)
        self.assertEqual(sum(len(f.surface_digests) for f in c.provider_families),288)

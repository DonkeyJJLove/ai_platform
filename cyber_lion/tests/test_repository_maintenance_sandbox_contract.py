...
def _operation(branch="mission/example", classification="A"):
    p = _policy()
...
def test_policy_is_exact_and_evidence_only():
    p = _policy()
    assert p.authority_effect is False
    assert p.master_effect is False
    assert len(p.digest()) == 64


@pytest.mark.parametrize("branch", ["master", "main", "refs/heads/x", "feature/x", "../x", "mission/../x"])
def test_protected_or_unsafe_branch_denied(branch):
    with pytest.raises(RepositoryMaintenanceContractError):
        _operation(branch=branch).validate()


def test_non_delete_classification_denied():
    with pytest.raises(RepositoryMaintenanceContractError):
        _operation(classification="C").validate()


def test_receipt_cannot_report_master_or_authority_effect():
    values = dict(
        schema_version="1.0.0",
        receipt_id="r1",
        operation_id="op1",
    ...
    bad = {**values, "authority_effect": True}
    with pytest.raises(RepositoryMaintenanceContractError):
        RepositoryMaintenanceExecutionReceipt.build(**bad)

...
function renderCluster(online=clusterOnline){
  ...
  const selectedFleet=latestRunFleets[run.run_id]||latestFleet;
  const current=online&&!historical&&selectedFleet.currentness==='OBSERVED'&&selectedFleet.run_ids?.length===1&&selectedFleet.run_ids[0]===run.run_id;
  const metrics=run.metrics||{};
  const records=current?(selectedFleet.pod_observations||[]).find(x=>x.run_id===run.run_id)?.pods:metrics.drone_pods;
  ...
  $('clusterPodDetail').hidden=true;
  const raw=current?selectedFleet.organizations:(metrics.fleet_organizations||{});
  let groups=Object.entries(raw||{}).map(([name,v])=>({name,total:countValue(current?v.total:v),active:current?countValue(v.active):null}));
  const reportedTotal=countValue(current?selectedFleet.fleet_total:(metrics.pods??run.workload?.pods));

...
def migrate(conn, now_fn):
    ...
    for name,ddl in (
        ...
        if name not in cols:
            conn.execute(f'ALTER TABLE mission_execution_assignments ADD COLUMN {name} {ddl}')
    ...
    conn.execute('INSERT OR IGNORE INTO mission_scheduler_migrations VALUES(6,?,?)',
                 ('lion.operator-stale-result-evidence/v1',stamp))
    if [r[0] for r in conn.execute('PRAGMA integrity_check')] != ['ok']:
        conn.rollback()
...
def eligible_missions(conn):
    ...
    # Operator containment is an admission fence, not advisory metadata.
    return [dict(r) for r in rows if operator_control.autonomy_allowed(conn, r['mission_id'])]
...
def create_assignment(conn, mission_id, phase_id, logical_drone_id, material_drone_id, input_value, now_fn, *, lease_generation, dispatch_authority="AUTONOMOUS"):
    if dispatch_authority not in {"AUTONOMOUS", operator_control.PRIMARY_OPERATOR}:
        raise ValueError("dispatch authority")
    control = operator_control.control_state(conn, mission_id, now_fn)
    if not operator_control.assignment_allowed(conn, mission_id, dispatch_authority, int(control["control_epoch"])):
        raise ValueError("operator control fence")
    capability = input_value.get("capability") if isinstance(input_value, dict) else None
    if isinstance(capability, str) and operator_control.is_capability_revoked(conn, mission_id, capability):
        raise ValueError("capability revoked by operator")
    aid = "assignment-" + uuid.uuid4().hex
    stamp = now_fn()
    conn.execute(
        "INSERT INTO mission_execution_assignments(assignment_id,mission_id,phase_id,logical_drone_id,material_drone_id,input_digest,input_json,state,lease_generation,control_epoch,context_revision,plan_revision,dispatch_authority,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (aid, mission_id, phase_id, logical_drone_id, material_drone_id, digest(input_value), _canon(input_value), "READY", int(lease_generation),
         int(control["control_epoch"]), int(control["context_revision"]), int(control["plan_revision"]), dispatch_authority, stamp),
    )
    conn.commit()
    return aid
...
class StaleAssignmentResult(ValueError):
    """Raised after a stale worker result is durably retained as historical evidence."""


def _store_stale_result(conn,row,result,result_digest,now_fn,*,material_drone_id,lease_generation,reason):
    if "mission_stale_assignment_results" not in {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}:
        raise ValueError(reason)
    control=operator_control.control_state(conn,row["mission_id"],None) or {}
    sid="stale-result-"+uuid.uuid4().hex;raw=_canon(result);stamp=now_fn()
    conn.execute("INSERT OR IGNORE INTO mission_stale_assignment_results VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
        (sid,row["assignment_id"],row["mission_id"],row["phase_id"],result_digest,raw,material_drone_id,lease_generation,
         int(row["control_epoch"] or 0),int(control.get("control_epoch") or 0),reason,stamp))
    conn.execute("UPDATE mission_execution_assignments SET state='STALE_RESULT',finished_at=COALESCE(finished_at,?) WHERE assignment_id=?",
...
def _record_receipt(conn, assignment_id, result, now_fn, *, status, effect_receipt_digest, authority_effect, worker_binding=None):
    ...
    try:
        ...
        row = conn.execute("SELECT assignment_id,mission_id,phase_id,state,material_drone_id,lease_generation,control_epoch,dispatch_authority FROM mission_execution_assignments WHERE assignment_id=?", (assignment_id,)).fetchone()
        if not row:
        ...
        if worker_binding is not None:
            ...
            if not operator_control.assignment_allowed(conn, row['mission_id'], row['dispatch_authority'], int(row['control_epoch'])):
                # Under the migrated operator-control plane a late result is not a
                # current decision, but it is still evidence. Persist it outside the
                # valid receipt ledger, terminalize the stale assignment, then fail
                # the caller closed so it cannot be mistaken for an accepted effect.
                if "mission_operator_control" in {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}:
                    sid=_store_stale_result(conn,row,result,result_digest,now_fn,material_drone_id=material_drone_id,lease_generation=lease_generation,reason='STALE_CONTROL_EPOCH')
                    conn.execute('RELEASE scheduler_receipt_ingress');conn.commit()
                    raise StaleAssignmentResult('stale assignment control epoch; historical_result='+sid)
            ...
            if row['lease_generation'] != lease_generation or not driver or driver['generation'] != lease_generation:
                ...
                if 'mission_operator_control' in tables and row['state']=='CANCEL_REQUESTED':
                    sid=_store_stale_result(conn,row,result,result_digest,now_fn,material_drone_id=material_drone_id,lease_generation=lease_generation,reason='STALE_DRIVER_GENERATION')
                    conn.execute('RELEASE scheduler_receipt_ingress');conn.commit()
                    raise StaleAssignmentResult('stale assignment generation; historical_result='+sid)
...
def pending_local_assignments(conn, *, mission_id=None, limit=16):
    ...
    allowed=[]
    for row in rows:
        if operator_control.assignment_allowed(conn, row['mission_id'], row['dispatch_authority'], int(row['control_epoch'])):
    ...
    try:
        ...
        if 'generation' in driver_cols:
            ...
            if not driver or int(driver['generation'])!=int(row['lease_generation']):
                raise ValueError('stale assignment generation')
        stamp=now_fn();expires=_assignment_lease_deadline(stamp,lease_seconds)
        cur=conn.execute("UPDATE mission_execution_assignments SET state='CLAIMED',claimed_at=?,lease_expires_at=? WHERE assignment_id=? AND state='READY'",(stamp,expires,assignment_id))
        if cur.rowcount!=1:raise ValueError("assignment claim race")
    except Exception:
        conn.execute('ROLLBACK TO assignment_claim_fence');conn.execute('RELEASE assignment_claim_fence');raise

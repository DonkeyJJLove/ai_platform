...
class EventSocket:
 ...
 def start(self):
  ...
  s=socket.socket(socket.AF_UNIX,socket.SOCK_STREAM);s.bind(self.path);os.chmod(self.path,0o660);s.listen(8);self.sock=s;threading.Thread(target=self._loop,daemon=True).start()
 def _loop(self):

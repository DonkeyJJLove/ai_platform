...
test('R18 integration A/B out-of-order completion and C deletion late receipt',async t=>{
  ...
  store.tombstoneThread(C.thread_id);phase=3;await reconciler.runOnce();assert.throws(()=>store.getThread(C.thread_id),/THREAD_NOT_FOUND/);assert.equal(store.reconciliation('rc').state,'ORPHANED_THREAD');assert.equal(store.debugState().receipts.find(x=>x.request_id==='rc').receipt_digest,'receipt-rc');
});

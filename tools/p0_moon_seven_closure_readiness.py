...
class SevenClosureReadinessReport:
    ...
    def validate(self):
        if self.mediated_count!=6 or self.partial_count!=1 or self.unknown_outside_seven_count!=331: raise MoonSevenClosureReadinessError("unexpected seven-surface readiness counts")
...
def materialize_seven_closure_readiness(*,inventory:EffectSurfaceInventory,taxonomy_report:EffectTaxonomyReconciliationReport,repo_root:Path)->SevenClosureReadinessArtifacts:
    ...
    if counts!={"MEDIATED":6,"PARTIAL":1,"UNMEDIATED":0,"UNKNOWN":331} or carrier.global_status!="UNKNOWN": raise MoonSevenClosureReadinessError("global carrier readiness counts drift")

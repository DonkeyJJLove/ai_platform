...
class SevenClosureReadinessReport:
    ...
    def validate(self):
        if self.mediated_count!=6 or self.partial_count!=1 or self.unknown_outside_seven_count!=250: raise MoonSevenClosureReadinessError("unexpected seven-surface readiness counts")
...
def materialize_seven_closure_readiness(*,inventory:EffectSurfaceInventory,taxonomy_report:EffectTaxonomyReconciliationReport,repo_root:Path)->SevenClosureReadinessArtifacts:
    ...
    if counts!={"MEDIATED":6,"PARTIAL":1,"UNMEDIATED":0,"UNKNOWN":252} or carrier.global_status!="UNKNOWN": raise MoonSevenClosureReadinessError("global carrier readiness counts drift")
    missing=tuple(sorted(f"{x.surface_digest}:{x.attack_id}" for x in readiness if x.classification!=CLASS_CANONICAL))
    report=SevenClosureReadinessReport(inventory.digest(),taxonomy_report.digest(),att.policy.digest(),structural.plan.digest(),tuple(sorted(mediation_closure_record_digest(r) for r in closure)),carrier.digest(),6,1,250,missing,"UNKNOWN",(f"assessment:{assessment.inventory_digest}","no-live-execution","no-test-as-bypass-result")).validate()

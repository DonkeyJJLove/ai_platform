...
$DirectSupervisorUrl = 'http://127.0.0.1:8768/status'
...
function Read-DirectSupervisorState {
    try { return Invoke-RestMethod -Uri $DirectSupervisorUrl -TimeoutSec 5 }
    catch { return $null }
}
function Test-DirectSupervisorReady {
    $state=Read-DirectSupervisorState
    return [bool]($state -and $state.state -eq 'READY' -and $state.credential_state -eq 'PRESENT' -and $state.inference_transport -eq 'OPENAI_RESPONSES_API_MEDIATED' -and $state.authority_effect -eq 'NONE')
}
...
function Write-State([hashtable]$RepoIdentity,[int]$ModelPid,[int]$PanelPid,[int]$MatHealthy) {
    ...
    $direct=Read-DirectSupervisorState
    $state=[ordered]@{
        ...
        direct_supervisor_8768=[bool]($direct -and $direct.state -eq 'READY')
        direct_supervisor_state=if($direct){[string]$direct.state}else{'UNKNOWN'}
        direct_supervisor_credential_state=if($direct){[string]$direct.credential_state}else{'UNKNOWN'}
        model_8772=$true
        panel_8780=$true
        legacy_browser_8790=$false
        browser_relay_active=$false
        inference_transport='OPENAI_RESPONSES_API_MEDIATED'
...
function One-Pass {
    ...
    if (-not (Test-OperatorControl)) { throw 'OPERATOR_CONTROL_8767_UNAVAILABLE' }
    if (-not (Test-DirectSupervisorReady)) { throw 'DIRECT_SUPERVISOR_8768_NOT_READY' }
    $panelPid=Ensure-Panel $repoIdentity
    Write-State $repoIdentity $modelPid $panelPid $mat
    Write-Log ('READY model=' + $modelPid + ' panel=' + $panelPid + ' direct8768=READY operator8767=READY browser8790=ABSENT mat=12 head=' + $repoIdentity.head)
}

...
def materialize_policy_v2_readiness(*,inventory:EffectSurfaceInventory,taxonomy_report:EffectTaxonomyReconciliationReport,repo_root:Path):
    ...
    if counts!={"MEDIATED":7,"PARTIAL":0,"UNMEDIATED":0,"UNKNOWN":252} or carrier.global_status!="UNKNOWN":raise MoonPermissionPolicyReclassificationError("v2 global counts drift")
    ...
    report=PermissionPolicyTopologyReadinessReportV2(inventory.digest(),taxonomy_report.digest(),policy.predecessor_policy_digest,policy.digest(),tuple(sorted(m.digest() for m in mappings)),tuple(sorted(x.digest() for x in policy.security_requirements)),tuple(sorted(mediation_closure_record_digest(x) for x in closure)),carrier.digest(),7,250,unresolved,next_plan,"UNKNOWN",("control-flow-source-proven","no-live-execution","no-security-requirement-dropped")).validate()

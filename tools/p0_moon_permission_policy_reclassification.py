...
def boundary_matrix(*,inventory:EffectSurfaceInventory,repo_root:Path,definitions)->Tuple[EffectBoundaryAttackMapping,...]:
    ...
    i_prepare=exec_src.index('self.fence.prepare(');i_second=exec_src.index('current_admission = self.admissions.resolve(request)');i_drift=exec_src.index('raise MoonFileWriteMediationError("authority drift")');i_unknown=exec_src.index('self.fence.mark_unknown(effect_key)')
    if not i_prepare<i_second<i_drift<i_unknown:raise MoonPermissionPolicyReclassificationError("stale authority boundary order drift")
    ...
    out.append(EffectBoundaryAttackMapping("STALE_AUTHORITY_SOURCE",PERMISSION_SURFACE,"DOWNSTREAM_CURRENTNESS_GUARD",d.pep_name,d.expected_denial,PERMISSION_ENTRYPOINT,"AFTER_SOURCE_EFFECT_AND_STATE_WRITE_BEFORE_DOWNSTREAM_EFFECT",OS_REPLACE_SURFACE,"DOWNSTREAM_CURRENTNESS_NEGATIVE_EVIDENCE","The second authority resolution occurs after the permission observation and durable fence prepare but before the certified os.replace target effect; authority drift is therefore a downstream currentness guard, not a bypass of permission observation.",(f"source:{MEDIATION_PATH}@{mediation_blob}","control-flow:fence.prepare-before-second-admissions.resolve-before-authority-drift",f"downstream-surface:{OS_REPLACE_SURFACE}",f"pep:{d.expected_pep_digest}")).validate())
    return tuple(out)
...
def policy_v2(*,inventory:EffectSurfaceInventory,repo_root:Path):
    ...
    sec=(
        ...
        RehomedSecurityRequirementV2("STALE_AUTHORITY_SOURCE",PERMISSION_SURFACE,"DOWNSTREAM_CURRENTNESS_GUARD",OS_REPLACE_SURFACE,bymap["STALE_AUTHORITY_SOURCE"].pep_name,bymap["STALE_AUTHORITY_SOURCE"].expected_denial,bymap["STALE_AUTHORITY_SOURCE"].effect_boundary_relation,"DOWNSTREAM_CURRENTNESS_NEGATIVE_EVIDENCE","CONTROL_FLOW_OBSERVED_EVIDENCE_REQUIRED","Authority freshness remains mandatory before the certified os.replace effect; the current implementation checks it only after a durable PREPARED write, so a no-effect negative receipt is not claimed.",bymap["STALE_AUTHORITY_SOURCE"].source_refs).validate(),
    )
...
def materialize_policy_v2_readiness(*,inventory:EffectSurfaceInventory,taxonomy_report:EffectTaxonomyReconciliationReport,repo_root:Path):
    ...
    next_plan=("UNTRUSTED_PERMISSION:extract-or-introduce-pure-admission-decision-boundary-before-negative-evidence","STALE_AUTHORITY_SOURCE:move-or-split-authority-revalidation-before-durable-fence-prepare-before-safe-negative-evidence")

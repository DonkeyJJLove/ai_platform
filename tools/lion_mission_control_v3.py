...
def saas_broker_api(method,path,payload=None):
 ...
 try:
  ...
  if method=='POST' and tail=='/requests':
   if type(x) is not dict or not {'scope_type','question','authority_effect'}<=set(x) or set(x)-{'scope_type','scope_id','thread_id','mission_id','question','authority_effect','control_transport','inference_transport','provider'}:raise ValueError('broker request schema')
   return saas_broker.create_request(c,x.get('mission_id'),x['question'],now,scope_type=x['scope_type'],scope_id=x.get('scope_id'),thread_id=x.get('thread_id'),authority_effect=x['authority_effect'],control_transport=x.get('control_transport',saas_broker.CONTROL_TRANSPORT),inference_transport=x.get('inference_transport'),provider=x.get('provider'))
  if method=='POST' and tail=='/mediator/heartbeat':
   return saas_broker.record_mediator_heartbeat(c,x,now)
  if method=='POST' and tail=='/direct/heartbeat':
   return saas_broker.record_direct_bridge_heartbeat(c,x,now)
  ...
  if method=='POST' and tail.startswith('/requests/'):
   ...
   if action=='respond':
    ...
    return saas_broker.respond(c,rid,x['response_token'],x['answer'],now,model_identity=x['model_identity'],transport=x['transport'],attestation_class=x['attestation_class'],claim_generation=x['claim_generation'],provider=x.get('provider'),provider_conversation_id=x.get('provider_conversation_id'),provider_response_id=x.get('provider_response_id'))

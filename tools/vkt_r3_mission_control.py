...
def main():
    ...
    sp=p.add_subparsers(dest='command',required=True); s=sp.add_parser('serve'); s.add_argument('--host',default='127.0.0.1'); s.add_argument('--port',type=int,default=8765); s.add_argument('--fallback-port',type=int,action='append',default=[]); s.add_argument('--listen-state',default='/run/lion-vkt-mission-control/listen.json'); sp.add_parser('status'); sp.add_parser('export'); sp.add_parser('verify')

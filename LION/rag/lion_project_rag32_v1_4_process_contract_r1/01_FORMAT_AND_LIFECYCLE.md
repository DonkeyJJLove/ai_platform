...
Reguła projektowa: MAX_PROJECT_ATTACHMENTS=40, HISTORICAL_CORE_ATTACHMENTS=32, ACTIVE_ATTACHMENTS=34, RESERVED_ATTACHMENTS=6. Rezerwa nie uprawnia do omijania inwentaryzacji. Tymczasowy evidence/delta może zajmować wolne miejsce jako materiał do oceny, lecz przed zamknięciem kolejnej epoki należy włączyć go do stabilnych kontenerów i zaktualizować ich manifest.
...
## Process-contract lifecycle

Zmiana phase intent nie może być promowana bez `PhaseExecutionContract`. Nowa kolejność wiedzy procesowej to `phase intent → execution contract → capability binding → execution spec → Action IR → evidence/completion reconciliation`. Kontrakt jest stabilnym semantic ownerem; runtime binding może być odświeżany bez przepisywania intencji fazy.

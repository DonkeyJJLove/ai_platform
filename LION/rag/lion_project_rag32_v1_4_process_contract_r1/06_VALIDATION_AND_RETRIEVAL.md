...
## Process Contract retrieval probe

Po instalacji zadaj: „Gdzie LION rozdziela phase intent, PhaseExecutionContract, capability binding, Action IR i completion?” Odpowiedź musi wskazać `33_LION_PROCESS_CONTRACT_PLANE__v1_4_process_contract_r1.md` oraz granicę `CAPABILITY_BINDING_IS_NOT_AUTHORITY`. Następnie zapytaj o LPCL/1.2 i `VERIFY_BEFORE_REPAIR`; wymagany jest również `32_CODEX_PROJECT_INTEGRATION.md` dla procedury Codex. To jest test routingu bieżącego control attachmentu, nie source_id z historycznych archiwów. `hosted_rag_result=NOT_RUN` do czasu rzeczywistego uploadu/retrieval.

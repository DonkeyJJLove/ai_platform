...
def verify(root: Path) -> tuple[dict[str, Any], dict[str, tuple[dict[str, Any], bytes]]]:
    ...
    if type(names) is not list or not 1 <= len(names) <= 40 or len(set(names)) != len(names):
        raise ValueError("profile requires 1..40 distinct attachments")
    if m.get("attachment_limit") != 40 or m.get("reserved_slots") != 40-len(names):
...
def extract(root: Path, out: Path) -> dict[str, Any]:
    ...
    carrier_paths = {spec["path"] for spec in m["carrier_specs"]}
    controls = {name: (root / name).read_text("utf-8") for name in m["attachment_files"]
                if name != MANIFEST and name not in carrier_paths}
...
def self_test(root: Path) -> dict[str, Any]:
    ...
    test("unexpected_unregistered_attachment", lambda d, m: (d / "UNREGISTERED.md").write_text("x"))
    ...
    with tempfile.TemporaryDirectory(prefix="lion-rag-roundtrip-") as t:
        ...
        checks.append({"test": "extract_rebuild_all_attachments_byte_identical", "status": "PASS"})

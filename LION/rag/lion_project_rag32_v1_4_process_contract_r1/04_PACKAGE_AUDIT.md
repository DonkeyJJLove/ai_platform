...
## Delta process-contract candidate

To wydanie dodaje dwa współczesne attachmenty repozytorium: 32 (Codex integration) i 33 (Process Contract Plane). Nie są one dopisywane do historycznych archiwów v13/v14c2 i nie zwiększają `source_count=123`. Warstwa 08–31 zachowuje oryginalne rekordy źródłowe bajtowo. Process Contract Plane ma `AUTHORITY_EFFECT=NONE`; jego obecność w RAG nie dowodzi wdrożenia runtime ani bieżącej liczby bound/unbound capabilities.

Korekta walidatora: profil od początku deklarował limit 40 i 8 rezerwowych slotów, lecz wcześniejszy repo-local validator wymuszał literalnie 32 attachments. Kandydat uzgadnia validator z manifestem: dokładny fileset pozostaje obowiązkowy, ale `1..40` attachments są legalne, a `reserved_slots=40-attachment_count`. Historyczne R1 nadal ma 32/8.

...
1. **Reacquire i route.** Ustal bieżący cel i istniejący zakres zgody. Odczytaj `LION/rag/lion_project_rag32_v1_4_r1/00_START_HERE.md`, `03_STATE_AND_CONTINUATION.md`, `02_ROUTING_AND_SOURCE_MAP.md`; wybierz pełne source records. Gdy zadanie zależy od currentness, odczytaj exact Git/CI/runtime w wymaganym zakresie. Niepełne źródło oznacza UNKNOWN dla zależnej decyzji.
...
4. **Execute tylko w zakresie.** Użyj `LION/codex/CODEX_RUNBOOK.md` i `TOOL_AUTHORITY_MAP.yaml`. Dostępność narzędzia nie nadaje authority. Respektuj zgodę już udzieloną w sesji, ograniczenia narzędzi i cel; nie żądaj ponownie tej samej zgody bez przyczyny. Merge, host i runtime effects wymagają własnego zakresu, którego ta umiejętność nie ustanawia. Nie obchodź odmowy interpreterem ani zmianą polityki.

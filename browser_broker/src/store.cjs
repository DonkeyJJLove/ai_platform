...
class Store{
 ...
 enqueue(value){
  ...
  return this.tx(()=>{
   ...
   const existing=this.rows(),active=existing.filter(r=>ACTIVE.includes(r.state));
   if(active.length>=18)throw Error('TASK_BUDGET_EXHAUSTED');
   if(active.some(r=>r.mission_id!==scope))throw Error('MISSION_LIMIT');
   if(active.filter(r=>r.mission_id===scope).length>=6)throw Error('TURN_LIMIT');
   if(active.some(r=>r.mission_id===scope&&(r.envelope.conversation_url!==v.conversation_url||r.envelope.panel_thread_id!==v.panel_thread_id)))throw Error('MISSION_BINDING_CONFLICT');
 ...
 refreshQueuedEnvelope(id,transform,reason='QUEUED_ENVELOPE_REFRESH'){return this.tx(()=>{
  const r=this.row(id);if(!r||r.state!=='QUEUED'||r.sends!==0)return false;
  const next=transform({...r.envelope});if(!next||typeof next!=='object')return false;
  const payload=JSON.stringify(next),digest=hash(payload);
  this.db.prepare('UPDATE wakes SET payload=?,digest=?,reason=?,updated_at=? WHERE request_id=?').run(payload,digest,reason,this.now(),id);
  this.record(id,'QUEUED',reason);return this.row(id);
 })}

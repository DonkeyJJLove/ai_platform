...
class ThreadConsumer{
 ...
 async admits(v){
  if(v.panel_thread_id!==this.scope.thread_id||v.conversation_url!==this.scope.conversation_url||v.task_sha256!==TASK)return false;
 ...
 async reconcile(){
  ...
  for(const job of this.store.rows().filter(j=>j.state==='RESULT_OBSERVED'&&j.envelope.panel_thread_id===this.scope.thread_id)){
   const v=job.envelope,row=await this.mc('/api/v3/saas-broker/requests/'+encodeURIComponent(v.request_id));
   if(row.request_id!==v.request_id||row.thread_id!==v.panel_thread_id||row.mission_id!==v.mission_id||row.scope_type!=='THREAD'||row.scope_id!==v.panel_thread_id)continue;
  ...
  try{
   ...
   for(const event of batch.events){
    ...
    if(event.type==='turn.pending'&&/^turn_[A-Za-z0-9-]+$/.test(event.data?.turn_id||'')){
     ...
     if(turn?.turn_id===event.data.turn_id&&turn.status==='PENDING'&&(turn.mission_id===undefined||turn.mission_id===null||typeof turn.mission_id==='string')&&turn.thread_id===this.scope.thread_id&&/^MC-saas-[A-Za-z0-9-]+$/.test(turn.command_id||'')){
      const rid=turn.command_id.slice(3),row=await this.mc('/api/v3/saas-broker/requests/'+encodeURIComponent(rid));
      if(row.scope_type!=='THREAD'||row.scope_id!==this.scope.thread_id||row.thread_id!==this.scope.thread_id||row.authority_effect!=='NONE'||row.transport!==TRANSPORT){this.cursor=event.seq;this.store.setSetting(this.cursorKey,event.seq);continue;}
      const missionId=(turn.mission_id===undefined||turn.mission_id===null&&row.mission_id!==null)?(row.mission_id??null):turn.mission_id;
      const v={request_id:rid,mission_id:missionId,panel_thread_id:this.scope.thread_id,turn_id:turn.turn_id,turn_request_hash:turn.request_hash,parent_event_id:turn.parent_event_id,conversation_url:this.scope.conversation_url,task_sha256:TASK,deadline_at:Math.min(Date.parse(row.deadline_at||row.expires_at),this.now()+1200000)};
